...
func ParseKeyID(keyID string) (backend, keyType, algo, keyname string, err error) {
	if keyID == "" {
		return "", "", "", "", &ErrValidation{Sentinel: ErrInvalidKeyIDFormat, Detail: "key ID cannot be empty"}
	}
	...
	if len(parts) != 4 {
		return "", "", "", "", &ErrValidation{
			Sentinel: ErrInvalidKeyIDFormat,
			Detail:   "expected format backend:type:algo:keyname, got " + keyID,
		}
	}
	...
	// Keyname is required
	if keyname == "" {
		return "", "", "", "", &ErrValidation{Sentinel: ErrInvalidKeyIDFormat, Detail: "keyname cannot be empty"}
	}
...
func validateBackend(backend string) error {
	if !validBackends[backend] {
		return &ErrValidation{Sentinel: ErrInvalidBackendType, Detail: backend}
	}
	return nil
}
...
func validateKeyType(keyType string) error {
	...
	return &ErrValidation{Sentinel: ErrInvalidKeyIDFormat, Detail: "invalid key type: " + keyType}
}
...
func validateAlgorithm(algo string) error {
	...
	return &ErrValidation{Sentinel: ErrInvalidKeyIDFormat, Detail: "invalid algorithm: " + algo}
}
...
func validateKeyName(keyname string) error {
	if keyname == "" {
		return &ErrValidation{Sentinel: ErrInvalidKeyName, Detail: "keyname cannot be empty"}
	}

	if len(keyname) > maxKeyNameLength {
		return &ErrValidation{
			Sentinel: ErrInvalidKeyName,
			Detail:   fmt.Sprintf("keyname too long (max %d characters)", maxKeyNameLength),
		}
	}

	// Check for path traversal
	if strings.Contains(keyname, "..") || strings.Contains(keyname, "/") || strings.Contains(keyname, "\\") {
		return &ErrValidation{Sentinel: ErrInvalidKeyName, Detail: "keyname cannot contain path traversal characters"}
	}
	...
	if !keynameRegex.MatchString(keyname) {
		return &ErrValidation{
			Sentinel: ErrInvalidKeyName,
			Detail:   "keyname must contain only alphanumeric, hyphens, underscores, and dots",
		}
	}
...
func keyIDToBackendType(backendStr string) (types.BackendType, error) {
	...
	default:
		return "", &ErrValidation{Sentinel: ErrInvalidBackendType, Detail: backendStr}
	}
}
...
func keyTypeToEnum(keyType string) (types.KeyType, error) {
	...
	default:
		return 0, &ErrUnsupportedKeyType{KeyType: keyType}
	}
}
...
func algorithmToEnum(algo string) (interface{}, error) {
	...
	case "chacha20-poly1305", "chacha20":
		return types.SymmetricChaCha20Poly1305, nil
	case "xchacha20-poly1305", "xchacha20":
		return types.SymmetricXChaCha20Poly1305, nil
	default:
...
func extractECDSACurve(algo string) (string, error) {
	...
	case strings.Contains(algo, "p521") || strings.Contains(algo, "p-521"):
		return "P-521", nil
	default:
		return "", &ErrUnsupportedECDSACurve{Algorithm: algo}
	}
}
