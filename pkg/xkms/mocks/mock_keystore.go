...
import (
	...
	storagemocks "github.com/jeremyhahn/go-xkms/pkg/storage/mocks"
	"github.com/jeremyhahn/go-xkms/pkg/types"
	"github.com/jeremyhahn/go-xkms/pkg/xkms"
)

// MockKeyStore is a mock implementation of xkms.Backend for testing.
...
func (m *MockKeyStore) GenerateRSA(attrs *types.KeyAttributes) (crypto.PrivateKey, error) {
	...
	if m.closed {
		return nil, xkms.ErrBackendClosed
	}
...
func (m *MockKeyStore) GetKey(attrs *types.KeyAttributes) (crypto.PrivateKey, error) {
	...
	if !ok {
		return nil, &xkms.ErrMockKeyNotFound{KeyID: attrs.CN}
	}
	return key, nil
}
...
func (m *MockKeyStore) DeleteKey(attrs *types.KeyAttributes) error {
	...
	if _, ok := m.keys[attrs.CN]; !ok {
		return &xkms.ErrMockKeyNotFound{KeyID: attrs.CN}
	}
...
func (m *MockKeyStore) Signer(attrs *types.KeyAttributes) (crypto.Signer, error) {
	...
	if !ok {
		return nil, xkms.ErrKeySigningNotSupported
	}
	return signer, nil
}
...
func (m *MockKeyStore) Decrypter(attrs *types.KeyAttributes) (crypto.Decrypter, error) {
	...
	if !ok {
		return nil, xkms.ErrKeyDecryptionNotSupported
	}
...
func (m *MockKeyStore) GetCert(keyID string) (*x509.Certificate, error) {
	...
	if !ok {
		return nil, &xkms.ErrMockCertNotFound{KeyID: keyID}
	}
	return cert, nil
}
...
func (m *MockKeyStore) DeleteCert(keyID string) error {
	...
	if _, ok := m.certs[keyID]; !ok {
		return &xkms.ErrMockCertNotFound{KeyID: keyID}
	}
...
func (m *MockKeyStore) GetCertChain(keyID string) ([]*x509.Certificate, error) {
	...
	if !ok {
		return nil, &xkms.ErrMockCertChainNotFound{KeyID: keyID}
	}
	return chain, nil
}
...
func (m *MockKeyStore) CertExists(keyID string) (bool, error) {
	...
	if m.closed {
		return false, xkms.ErrBackendClosed
	}
...
func (m *MockKeyStore) GetTLSCertificate(keyID string, attrs *types.KeyAttributes) (tls.Certificate, error) {
	...
	if m.closed {
		return tls.Certificate{}, xkms.ErrBackendClosed
	}
	...
	key, ok := m.keys[keyID]
	if !ok {
		return tls.Certificate{}, &xkms.ErrMockKeyNotFound{KeyID: keyID}
	}

	cert, ok := m.certs[keyID]
	if !ok {
		return tls.Certificate{}, &xkms.ErrMockCertNotFound{KeyID: keyID}
	}
...
func (m *MockKeyStore) GetKeyByID(keyID string) (crypto.PrivateKey, error) {
	...
	key, ok := m.keys[keyID]
	if !ok {
		return nil, &xkms.ErrMockKeyNotFound{KeyID: keyID}
	}
	return key, nil
}
