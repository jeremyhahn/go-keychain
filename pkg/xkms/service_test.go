...
// mockKeyStore is a mock implementation of Backend for testing
type mockKeyStore struct {
	name        string
	keys        map[string]crypto.PrivateKey
	keyAttrs    map[string]*types.KeyAttributes
	certs       map[string]*x509.Certificate
	backend     types.KeyProvider
	closeError  error
	listKeysErr error
}
...
func (m *mockKeyStore) KeyProvider() types.KeyProvider {
	return m.backend
}
...
func TestInitialize_EmptyBackends(t *testing.T) {
	...
	err := Initialize(config)
	require.Error(t, err)
	assert.True(t, errors.Is(err, ErrNoBackendsConfigured))
}
...
func TestGetBackend_NotInitialized(t *testing.T) {
	Reset()

	_, err := GetBackend("software")
	assert.Error(t, err)
...
func TestKeyProviders_Success(t *testing.T) {
	Reset()
	t.Cleanup(func() { Reset() })

	mockProvider := newMockKeyStore("pkcs8")
	config := &ServiceConfig{
		Backends: map[string]Backend{
			"software": newMockKeyStore("software"),
		},
		KeyProviders: map[string]Backend{
			"pkcs8":     mockProvider,
			"symmetric": newMockKeyStore("symmetric"),
		},
		DefaultBackend: "software",
	}
	err := Initialize(config)
	...
	assert.Contains(t, providers, "pkcs8")
	assert.Contains(t, providers, "symmetric")

	// Backends should NOT contain key providers
	backends := Backends()
	assert.NotContains(t, backends, "pkcs8")
	assert.NotContains(t, backends, "symmetric")
	assert.Contains(t, backends, "software")
}

func TestKeyProviders_NotInitialized(t *testing.T) {
	Reset()
...
func TestKeyProviders_NilMap(t *testing.T) {
	Reset()
	t.Cleanup(func() { Reset() })
...
func TestGetKeyProvider_Success(t *testing.T) {
	Reset()
	t.Cleanup(func() { Reset() })
	...
	kp, err := GetKeyProvider("pkcs8")
	require.NoError(t, err)
	assert.NotNil(t, kp)
}

func TestGetKeyProvider_NotFound(t *testing.T) {
	Reset()
	t.Cleanup(func() { Reset() })
	...
	_, err = GetKeyProvider("nonexistent")
	assert.Error(t, err)
	assert.ErrorIs(t, err, ErrKeyProviderNotFound)
}

func TestGetKeyProvider_NotInitialized(t *testing.T) {
	Reset()

	_, err := GetKeyProvider("pkcs8")
	assert.ErrorIs(t, err, ErrNotInitialized)
}
...
func (m *mockExtendedBackend) ExportKeyMaterial(attrs *types.KeyAttributes) ([]byte, error) {
	// Return mock key material for symmetric keys
	key, ok := m.symKeys[attrs.CN]
	if !ok {
		return nil, backend.ErrKeyNotFound
	}
...
// mockExtendedKeyStore wraps mockExtendedBackend to implement Backend
...
func (m *mockExtendedKeyStore) KeyProvider() types.KeyProvider {
...
// ========================================================================
// Test setter methods and accessors for optional subsystems
// ========================================================================

func TestSetBarrier_AndGet(t *testing.T) {
	setupService(t)
	svc, err := Get()
	require.NoError(t, err)
	...
	// We can't easily create a real Barrier without storage/strategy,
	// but we can set nil and verify it stays nil
	svc.SetBarrier(nil)
	assert.Nil(t, svc.barrier)
}

func TestSetWebAuthn_NilSet(t *testing.T) {
	setupService(t)
	svc, err := Get()
	require.NoError(t, err)

	// Initially nil
	assert.Nil(t, svc.webauthnService)
...
func TestCeremonyService_NilByDefault(t *testing.T) {
	setupService(t)
	svc, err := Get()
	require.NoError(t, err)

	// CeremonyService should return nil by default
	result := svc.CeremonyService()
	assert.Nil(t, result)
}

func TestSetCeremonyService_AndGet(t *testing.T) {
	setupService(t)
	svc, err := Get()
	require.NoError(t, err)

	// Set a dummy ceremony service (stored as any)
	type mockCeremony struct{ Name string }
	mock := &mockCeremony{Name: "test-ceremony"}
	svc.SetCeremonyService(mock)

	// Retrieve and type-assert
	result := svc.CeremonyService()
	require.NotNil(t, result)
	mc, ok := result.(*mockCeremony)
	require.True(t, ok)
	assert.Equal(t, "test-ceremony", mc.Name)
}

func TestCA_NilByDefault(t *testing.T) {
	setupService(t)
	svc, err := Get()
	require.NoError(t, err)

	// CA should be nil by default
	assert.Nil(t, svc.CA())
}

func TestSetCA_NilSet(t *testing.T) {
	setupService(t)
	svc, err := Get()
	require.NoError(t, err)
...
// ========================================================================
// Test Error Types - Error() and Unwrap() coverage
// ========================================================================

func TestErrBackendUnsupported_Error_AllBranches(t *testing.T) {
	// With backend and detail
	e1 := &ErrBackendUnsupported{
		Sentinel: ErrOperationNotSupported,
		Backend:  "tpm2",
		Detail:   "import not available",
	}
	assert.Contains(t, e1.Error(), "tpm2")
	assert.Contains(t, e1.Error(), "import not available")
	assert.ErrorIs(t, e1, ErrOperationNotSupported)

	// With backend only
	e2 := &ErrBackendUnsupported{
		Sentinel: ErrOperationNotSupported,
		Backend:  "pkcs11",
	}
	assert.Contains(t, e2.Error(), "pkcs11")
	assert.NotContains(t, e2.Error(), "backend pkcs11:")

	// Without backend or detail (sentinel only)
	e3 := &ErrBackendUnsupported{
		Sentinel: ErrOperationNotSupported,
	}
	assert.Equal(t, ErrOperationNotSupported.Error(), e3.Error())
	assert.ErrorIs(t, e3, ErrOperationNotSupported)
}

func TestErrOperationWrap_Error_AllBranches(t *testing.T) {
	// With detail and err
	innerErr := errors.New("inner error")
	e1 := &ErrOperationWrap{
		Sentinel: ErrSigningFailed,
		Detail:   "signing RSA",
		Err:      innerErr,
	}
	assert.Contains(t, e1.Error(), "signing RSA")
	assert.Contains(t, e1.Error(), "inner error")
	assert.ErrorIs(t, e1, ErrSigningFailed)
	assert.ErrorIs(t, e1, innerErr)
	...
	assert.Contains(t, e2.Error(), "no signer")
	...
	assert.Equal(t, ErrSigningFailed.Error(), e3.Error())

	// Unwrap returns both sentinel and err
	unwrapped := e1.Unwrap()
	assert.Len(t, unwrapped, 2)

	// Unwrap with nil err returns only sentinel
	unwrapped2 := e2.Unwrap()
	assert.Len(t, unwrapped2, 1)
}

func TestErrKeyIDParse_ErrorAndUnwrap(t *testing.T) {
	inner := errors.New("bad format")
	e := &ErrKeyIDParse{Err: inner}
	assert.Contains(t, e.Error(), "invalid key ID")
	assert.Contains(t, e.Error(), "bad format")
	assert.Equal(t, inner, e.Unwrap())
}

func TestErrBackendNameValidation_ErrorAndUnwrap(t *testing.T) {
	inner := errors.New("contains slashes")
	e := &ErrBackendNameValidation{Err: inner}
	assert.Contains(t, e.Error(), "invalid backend name")
	assert.Contains(t, e.Error(), "contains slashes")
	assert.Equal(t, inner, e.Unwrap())
}

func TestErrKeyProviderNameValidation_ErrorAndUnwrap(t *testing.T) {
	inner := errors.New("too long")
	e := &ErrKeyProviderNameValidation{Err: inner}
	assert.Contains(t, e.Error(), "invalid key provider name")
	assert.Contains(t, e.Error(), "too long")
	assert.Equal(t, inner, e.Unwrap())
}

func TestErrCertOperation_ErrorAndUnwrap(t *testing.T) {
	inner := errors.New("storage full")
	e := &ErrCertOperation{Operation: "save certificate", Err: inner}
	assert.Contains(t, e.Error(), "failed to save certificate")
	assert.Contains(t, e.Error(), "storage full")
	assert.Equal(t, inner, e.Unwrap())
}

func TestErrKeyOperation_Error_AllBranches(t *testing.T) {
	inner := errors.New("timeout")
	// With KeyID
	e1 := &ErrKeyOperation{Operation: "delete", KeyID: "my-key", Err: inner}
	assert.Contains(t, e1.Error(), "delete key my-key")
	assert.Contains(t, e1.Error(), "timeout")
	assert.Equal(t, inner, e1.Unwrap())

	// Without KeyID
	e2 := &ErrKeyOperation{Operation: "generate", Err: inner}
	assert.Contains(t, e2.Error(), "failed to generate")
	assert.NotContains(t, e2.Error(), "key :")
}

func TestErrCryptoOperation_Error_AllBranches(t *testing.T) {
	inner := errors.New("HSM error")
	// With KeyID
	e1 := &ErrCryptoOperation{Operation: "signer", KeyID: "rsa-key", Err: inner}
	assert.Contains(t, e1.Error(), "signer for key rsa-key")
	assert.Contains(t, e1.Error(), "HSM error")
	assert.Equal(t, inner, e1.Unwrap())

	// Without KeyID
	e2 := &ErrCryptoOperation{Operation: "decrypter", Err: inner}
	assert.Contains(t, e2.Error(), "failed to get decrypter")
	assert.NotContains(t, e2.Error(), "key ")
}

func TestErrCAOperation_ErrorAndUnwrap(t *testing.T) {
	inner := errors.New("cert expired")
	e := &ErrCAOperation{Operation: "issue", Err: inner}
	assert.Contains(t, e.Error(), "ca: issue")
	assert.Contains(t, e.Error(), "cert expired")
	assert.Equal(t, inner, e.Unwrap())
}

func TestErrUserOperation_ErrorAndUnwrap(t *testing.T) {
	inner := errors.New("duplicate user")
	e := &ErrUserOperation{Operation: "create user", Err: inner}
	assert.Contains(t, e.Error(), "keystore: failed to create user")
	assert.Contains(t, e.Error(), "duplicate user")
	assert.Equal(t, inner, e.Unwrap())
}

func TestErrSealOperation_ErrorAndUnwrap(t *testing.T) {
	inner := errors.New("wrong key")
	e := &ErrSealOperation{Operation: "unseal", Err: inner}
	assert.Contains(t, e.Error(), "failed to unseal data")
	assert.Contains(t, e.Error(), "wrong key")
	assert.Equal(t, inner, e.Unwrap())
}

func TestErrSealBackendMismatch_ErrorAndUnwrap(t *testing.T) {
	e := &ErrSealBackendMismatch{
		SealedBy: types.BackendTypeTPM2,
		Current:  types.BackendTypeSoftware,
	}
	assert.Contains(t, e.Error(), "sealed data was created by")
	assert.Contains(t, e.Error(), string(types.BackendTypeTPM2))
	assert.Contains(t, e.Error(), string(types.BackendTypeSoftware))
	assert.ErrorIs(t, e, ErrBackendMismatch)
}

func TestErrBackendClose_ErrorAndUnwrap(t *testing.T) {
	errs := []error{errors.New("err1"), errors.New("err2")}
	e := &ErrBackendClose{Errs: errs}
	assert.Contains(t, e.Error(), "failed to close 2 backend(s)")
	unwrapped := e.Unwrap()
	assert.Len(t, unwrapped, 2)
}

func TestErrBackendCloseItem_ErrorAndUnwrap(t *testing.T) {
	inner := errors.New("connection refused")
	e := &ErrBackendCloseItem{Backend: "vault", Err: inner}
	assert.Contains(t, e.Error(), "backend vault")
	assert.Contains(t, e.Error(), "connection refused")
	assert.Equal(t, inner, e.Unwrap())
}

func TestErrPEMBlock_ErrorAndUnwrap(t *testing.T) {
	e := &ErrPEMBlock{Expected: "CERTIFICATE", Got: "RSA PRIVATE KEY"}
	assert.Contains(t, e.Error(), "expected CERTIFICATE block")
	assert.Contains(t, e.Error(), "got RSA PRIVATE KEY")
	assert.ErrorIs(t, e, ErrInvalidEncodingPEM)
}

func TestErrCertParseIndex_ErrorAndUnwrap(t *testing.T) {
	inner := errors.New("ASN.1 parse error")
	e := &ErrCertParseIndex{Index: 3, Err: inner}
	assert.Contains(t, e.Error(), "index 3")
	assert.Contains(t, e.Error(), "ASN.1 parse error")
	assert.Equal(t, inner, e.Unwrap())
}

func TestErrUnsupportedKeyType_Error(t *testing.T) {
	e := &ErrUnsupportedKeyType{KeyType: "quantum-lattice"}
	assert.Contains(t, e.Error(), "unknown key type: quantum-lattice")
}

func TestErrUnsupportedAlgorithm_Error(t *testing.T) {
	e := &ErrUnsupportedAlgorithm{Algorithm: "NTRU"}
	assert.Contains(t, e.Error(), "unknown algorithm: NTRU")
}

func TestErrUnsupportedECDSACurve_Error(t *testing.T) {
	e := &ErrUnsupportedECDSACurve{Algorithm: "ECDSA-P999"}
	assert.Contains(t, e.Error(), "unknown ECDSA curve in algorithm: ECDSA-P999")
}

func TestErrUnsupportedPrivateKeyType_Error(t *testing.T) {
	e := &ErrUnsupportedPrivateKeyType{KeyType: "*dsa.PrivateKey"}
	assert.Contains(t, e.Error(), "unsupported private key type: *dsa.PrivateKey")
}

func TestErrPublicKeyExtraction_Error(t *testing.T) {
	e := &ErrPublicKeyExtraction{Algorithm: "ML-KEM-768"}
	assert.Contains(t, e.Error(), "failed to extract public key from generated ML-KEM-768 key")
}

func TestErrCopyKeyBackend_ErrorAndUnwrap(t *testing.T) {
	inner := errors.New("backend offline")
	e := &ErrCopyKeyBackend{Role: "destination", Err: inner}
	assert.Contains(t, e.Error(), "destination backend")
	assert.Contains(t, e.Error(), "backend offline")
	assert.Equal(t, inner, e.Unwrap())
}

func TestErrUnsupportedPublicKeyAlgorithm_ErrorAndUnwrap(t *testing.T) {
	e := &ErrUnsupportedPublicKeyAlgorithm{Algorithm: x509.DSA}
	assert.Contains(t, e.Error(), "DSA")
	assert.ErrorIs(t, e, ErrUnsupportedKeyAlgorithm)
}

func TestErrUnsealBackendNotFound_ErrorAndUnwrap(t *testing.T) {
	e := &ErrUnsealBackendNotFound{BackendType: types.BackendTypeTPM2}
	assert.Contains(t, e.Error(), "no backend found for type")
	assert.Contains(t, e.Error(), string(types.BackendTypeTPM2))
	assert.ErrorIs(t, e, ErrBackendNotFound)
}

func TestErrImportExportOperation_ErrorAndUnwrap(t *testing.T) {
	inner := errors.New("key too large")
	e := &ErrImportExportOperation{Operation: "import key", Err: inner}
	assert.Contains(t, e.Error(), "failed to import key")
	assert.Contains(t, e.Error(), "key too large")
	assert.Equal(t, inner, e.Unwrap())
}

func TestErrMockKeyNotFound_ErrorAndUnwrap(t *testing.T) {
	e := &ErrMockKeyNotFound{KeyID: "test-key-123"}
	assert.Contains(t, e.Error(), "test-key-123")
	assert.ErrorIs(t, e, ErrKeyNotFound)
}

func TestErrMockCertNotFound_ErrorAndUnwrap(t *testing.T) {
	e := &ErrMockCertNotFound{KeyID: "cert-key-456"}
	assert.Contains(t, e.Error(), "cert-key-456")
	assert.ErrorIs(t, e, ErrCertNotFound)
}

func TestErrMockCertChainNotFound_ErrorAndUnwrap(t *testing.T) {
	e := &ErrMockCertChainNotFound{KeyID: "chain-key-789"}
	assert.Contains(t, e.Error(), "chain-key-789")
	assert.ErrorIs(t, e, ErrCertChainNotFound)
}

// --- ErrValidation empty Detail ---

func TestErrValidation_EmptyDetail(t *testing.T) {
	e := &ErrValidation{Sentinel: ErrInvalidKeyAttributes}
	// With empty Detail, Error() should return just the sentinel message
	assert.Equal(t, ErrInvalidKeyAttributes.Error(), e.Error())
	assert.ErrorIs(t, e, ErrInvalidKeyAttributes)
}

// --- ErrBackendLookup empty Name ---

func TestErrBackendLookup_EmptyName(t *testing.T) {
	e := &ErrBackendLookup{Sentinel: ErrBackendNotFound}
	// With empty Name, Error() should return just the sentinel message
	assert.Equal(t, ErrBackendNotFound.Error(), e.Error())
	assert.ErrorIs(t, e, ErrBackendNotFound)
}

// --- validateTenantAccess error path for attr-based wrappers ---

func TestSigner_InvalidTenantAccess(t *testing.T) {
	setupService(t)
	_, err := Signer(&types.KeyAttributes{CN: "key", TenantID: "../evil"})
	require.Error(t, err)
}

func TestDecrypter_InvalidTenantAccess(t *testing.T) {
	setupService(t)
	_, err := Decrypter(&types.KeyAttributes{CN: "key", TenantID: "../evil"})
	require.Error(t, err)
}

func TestDeleteKey_InvalidTenantAccess(t *testing.T) {
	setupService(t)
	err := DeleteKey(&types.KeyAttributes{CN: "key", TenantID: "../evil"})
	require.Error(t, err)
}

func TestCertificate_InvalidTenantAccess(t *testing.T) {
	setupService(t)
	_, err := Certificate(&types.KeyAttributes{CN: "key", TenantID: "../evil"})
	require.Error(t, err)
}

func TestSaveCertificate_InvalidTenantAccess(t *testing.T) {
	setupService(t)
	err := SaveCertificate(&types.KeyAttributes{CN: "key", TenantID: "../evil"}, nil)
	require.Error(t, err)
}

func TestDeleteCertificate_InvalidTenantAccess(t *testing.T) {
	setupService(t)
	err := DeleteCertificate(&types.KeyAttributes{CN: "key", TenantID: "../evil"})
	require.Error(t, err)
}

func TestCertificateChain_InvalidTenantAccess(t *testing.T) {
	setupService(t)
	_, err := CertificateChain(&types.KeyAttributes{CN: "key", TenantID: "../evil"})
	require.Error(t, err)
}

func TestSaveCertificateChain_InvalidTenantAccess(t *testing.T) {
	setupService(t)
	err := SaveCertificateChain(&types.KeyAttributes{CN: "key", TenantID: "../evil"}, nil)
	require.Error(t, err)
}

func TestCertificateExists_InvalidTenantAccess(t *testing.T) {
	setupService(t)
	_, err := CertificateExists(&types.KeyAttributes{CN: "key", TenantID: "../evil"})
	require.Error(t, err)
}

func TestTLSCertificate_InvalidTenantAccess(t *testing.T) {
	setupService(t)
	_, err := TLSCertificate(&types.KeyAttributes{CN: "key", TenantID: "../evil"})
	require.Error(t, err)
}

// --- ValidateKeyReference error path for ByID wrappers ---

func TestSignerByID_EmptyKeyID(t *testing.T) {
	setupService(t)
	_, err := SignerByID("")
	require.Error(t, err)
}

func TestDecrypterByID_EmptyKeyID(t *testing.T) {
	setupService(t)
	_, err := DecrypterByID("")
	require.Error(t, err)
}

func TestDeleteKeyByID_EmptyKeyID(t *testing.T) {
	setupService(t)
	err := DeleteKeyByID("")
	require.Error(t, err)
}

func TestSaveCertificateByID_EmptyKeyID(t *testing.T) {
	setupService(t)
	err := SaveCertificateByID("", nil)
	require.Error(t, err)
}

func TestCertificateByID_EmptyKeyID(t *testing.T) {
	setupService(t)
	_, err := CertificateByID("")
	require.Error(t, err)
}

func TestDeleteCertificateByID_EmptyKeyID(t *testing.T) {
	setupService(t)
	err := DeleteCertificateByID("")
	require.Error(t, err)
}

// --- More validateTenantAccess coverage ---

func TestGenerateKeyWithBackend_InvalidTenantAccess(t *testing.T) {
	setupService(t)
...
func TestRotateKey_EmptyKeyID_Validation(t *testing.T) {
	setupService(t)
	_, err := RotateKey("")
	require.Error(t, err)
}

func TestSign_PackageFunc_EmptyKeyID(t *testing.T) {
	setupService(t)
	_, err := Sign("", []byte("data"), nil)
	require.Error(t, err)
}

func TestVerify_PackageFunc_EmptyKeyID(t *testing.T) {
	setupService(t)
	err := Verify("", []byte("data"), []byte("sig"), nil)
	require.Error(t, err)
}

func TestSealWithBackend_InvalidBackendName(t *testing.T) {
	setupService(t)
	_, err := SealWithBackend(context.Background(), "../evil", []byte("data"), nil)
	require.Error(t, err)
}

func TestUnsealWithBackend_InvalidBackendName(t *testing.T) {
	setupService(t)
	_, err := UnsealWithBackend(context.Background(), "../evil", nil, nil)
	require.Error(t, err)
}

// --- Service.go wrapper functions with additional patterns ---

func TestSaveCertificateChainByID_EmptyKeyID(t *testing.T) {
	setupService(t)
	err := SaveCertificateChainByID("", nil)
	require.Error(t, err)
}

func TestCertificateChainByID_EmptyKeyID(t *testing.T) {
	setupService(t)
	_, err := CertificateChainByID("")
	require.Error(t, err)
}

func TestCertificateExistsByID_EmptyKeyID(t *testing.T) {
	setupService(t)
	_, err := CertificateExistsByID("")
	require.Error(t, err)
}

func TestTLSCertificateByID_EmptyKeyID(t *testing.T) {
	setupService(t)
	_, err := TLSCertificateByID("")
	require.Error(t, err)
}

func TestGetSymmetricKey_EmptyKeyID(t *testing.T) {
	setupService(t)
	_, err := GetSymmetricKey("")
	require.Error(t, err)
}

func TestGenerateSymmetricKey_InvalidTenantAccess(t *testing.T) {
	setupService(t)
	_, err := GenerateSymmetricKey("software", &types.KeyAttributes{
		CN:       "key",
		TenantID: "../evil",
	})
	require.Error(t, err)
...
// --- BackendFor error path for wrapper functions ---
// These tests exercise the error from BackendFor when StoreType
// doesn't match any registered backend.

func TestSigner_InvalidStoreType(t *testing.T) {
	setupService(t)
	_, err := Signer(&types.KeyAttributes{CN: "key", StoreType: "nonexistent"})
	require.Error(t, err)
}

func TestDecrypter_InvalidStoreType(t *testing.T) {
	setupService(t)
	_, err := Decrypter(&types.KeyAttributes{CN: "key", StoreType: "nonexistent"})
	require.Error(t, err)
}

func TestDeleteKey_InvalidStoreType(t *testing.T) {
	setupService(t)
	err := DeleteKey(&types.KeyAttributes{CN: "key", StoreType: "nonexistent"})
	require.Error(t, err)
}

func TestCertificate_InvalidStoreType(t *testing.T) {
	setupService(t)
	_, err := Certificate(&types.KeyAttributes{CN: "key", StoreType: "nonexistent"})
	require.Error(t, err)
}

func TestSaveCertificate_InvalidStoreType(t *testing.T) {
	setupService(t)
	err := SaveCertificate(&types.KeyAttributes{CN: "key", StoreType: "nonexistent"}, nil)
	require.Error(t, err)
}

func TestDeleteCertificate_InvalidStoreType(t *testing.T) {
	setupService(t)
	err := DeleteCertificate(&types.KeyAttributes{CN: "key", StoreType: "nonexistent"})
	require.Error(t, err)
}

func TestCertificateChain_InvalidStoreType(t *testing.T) {
	setupService(t)
	_, err := CertificateChain(&types.KeyAttributes{CN: "key", StoreType: "nonexistent"})
	require.Error(t, err)
}

func TestSaveCertificateChain_InvalidStoreType(t *testing.T) {
	setupService(t)
	err := SaveCertificateChain(&types.KeyAttributes{CN: "key", StoreType: "nonexistent"}, nil)
	require.Error(t, err)
}

func TestCertificateExists_InvalidStoreType(t *testing.T) {
	setupService(t)
	_, err := CertificateExists(&types.KeyAttributes{CN: "key", StoreType: "nonexistent"})
	require.Error(t, err)
}

func TestTLSCertificate_InvalidStoreType(t *testing.T) {
	setupService(t)
	_, err := TLSCertificate(&types.KeyAttributes{CN: "key", StoreType: "nonexistent"})
	require.Error(t, err)
}

// --- getBackendForKID error path for ByID wrappers ---
// These test with a KID that references a nonexistent backend.

func TestSignerByID_NonexistentBackend(t *testing.T) {
	setupService(t)
	_, err := SignerByID("nonexistent:::some-key")
	require.Error(t, err)
}

func TestDecrypterByID_NonexistentBackend(t *testing.T) {
	setupService(t)
	_, err := DecrypterByID("nonexistent:::some-key")
	require.Error(t, err)
}

func TestDeleteKeyByID_NonexistentBackend(t *testing.T) {
	setupService(t)
	err := DeleteKeyByID("nonexistent:::some-key")
	require.Error(t, err)
}

func TestSaveCertificateByID_NonexistentBackend(t *testing.T) {
	setupService(t)
	err := SaveCertificateByID("nonexistent:::some-key", nil)
	require.Error(t, err)
}

func TestCertificateByID_NonexistentBackend(t *testing.T) {
	setupService(t)
	_, err := CertificateByID("nonexistent:::some-key")
	require.Error(t, err)
}

func TestDeleteCertificateByID_NonexistentBackend(t *testing.T) {
	setupService(t)
	err := DeleteCertificateByID("nonexistent:::some-key")
	require.Error(t, err)
}

func TestRotateKey_NonexistentBackend(t *testing.T) {
	setupService(t)
	_, err := RotateKey("nonexistent:::some-key")
	require.Error(t, err)
}

func TestSaveCertificateChainByID_NonexistentBackend(t *testing.T) {
	setupService(t)
	err := SaveCertificateChainByID("nonexistent:::some-key", nil)
	require.Error(t, err)
}

func TestCertificateChainByID_NonexistentBackend(t *testing.T) {
	setupService(t)
	_, err := CertificateChainByID("nonexistent:::some-key")
	require.Error(t, err)
}

func TestCertificateExistsByID_NonexistentBackend(t *testing.T) {
	setupService(t)
	_, err := CertificateExistsByID("nonexistent:::some-key")
	require.Error(t, err)
}

func TestTLSCertificateByID_NonexistentBackend(t *testing.T) {
	setupService(t)
	_, err := TLSCertificateByID("nonexistent:::some-key")
	require.Error(t, err)
}

func TestSign_PackageFunc_NonexistentBackend(t *testing.T) {
	setupService(t)
	_, err := Sign("nonexistent:::some-key", []byte("data"), nil)
	require.Error(t, err)
}

func TestVerify_PackageFunc_NonexistentBackend(t *testing.T) {
	setupService(t)
	err := Verify("nonexistent:::some-key", []byte("data"), []byte("sig"), nil)
	require.Error(t, err)
}

func TestEncrypt_PackageFunc_EmptyKeyID(t *testing.T) {
	setupService(t)
	_, err := Encrypt("", []byte("data"), nil)
	require.Error(t, err)
}

func TestDecrypt_PackageFunc_EmptyKeyID(t *testing.T) {
	setupService(t)
	_, err := Decrypt("", nil, nil)
	require.Error(t, err)
}

func TestGetImportParameters_PackageFunc_InvalidBackend(t *testing.T) {
	setupService(t)
	_, err := GetImportParameters("../evil", nil, "")
	require.Error(t, err)
}

func TestWrapKey_InvalidBackend(t *testing.T) {
	setupService(t)
	_, err := WrapKey("../evil", nil, nil)
	require.Error(t, err)
}

func TestUnwrapKey_InvalidBackend(t *testing.T) {
	setupService(t)
	_, err := UnwrapKey("../evil", nil, nil)
	require.Error(t, err)
}

func TestImportKey_InvalidBackend(t *testing.T) {
	setupService(t)
	err := ImportKey("../evil", nil, nil)
	require.Error(t, err)
}

func TestExportKey_PackageFunc_EmptyKeyID(t *testing.T) {
	setupService(t)
	_, err := ExportKey("", "")
	require.Error(t, err)
}

func TestCopyKey_EmptyKeyID(t *testing.T) {
	setupService(t)
	err := CopyKey("", "", nil)
	require.Error(t, err)
}
