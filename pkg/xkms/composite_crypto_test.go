...
import (
	...
	backmocks "github.com/jeremyhahn/go-xkms/pkg/backend/mocks"
...
func TestCompositeKeyStore_Signer(t *testing.T) {
	tests := []struct {
		name         string
		setupBackend func(*xkms.Backend)
		attrs        *types.KeyAttributes
		expectError  bool
	}{
		{
			name: "valid signer",
			setupBackend: func(ks *xkms.Backend) {
				attrs := &types.KeyAttributes{
					CN:           "test-signer",
...
func TestCompositeKeyStore_GetTLSCertificate(t *testing.T) {
	tests := []struct {
		name          string
		setupKeyStore func(*xkms.Backend)
		keyID         string
		attrs         *types.KeyAttributes
		expectError   bool
	}{
		{
			...
			setupKeyStore: func(ks *xkms.Backend) {
				attrs := &types.KeyAttributes{
					CN:           "test-tls",
					KeyType:      backend.KEY_TYPE_TLS,
					StoreType:    backend.STORE_SW,
...
func TestCompositeKeyStore_GetTLSCertificate_EmptyChain(t *testing.T) {
	...
	mockStorage.GetCertChainFunc = func(id string) ([]*x509.Certificate, error) {
		...
		return nil, xkms.ErrCertNotFound
	}
	...
	mockBackend.GetKeyFunc = func(attrs *types.KeyAttributes) (crypto.PrivateKey, error) {
		...
		return nil, xkms.ErrKeyNotFound
	}
