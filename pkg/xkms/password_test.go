...
func TestNewPassword(t *testing.T) {
	...
	p2 := NewPassword(originalPassword)
	originalPassword[0] = 'X' // Modify original

	str2, _ := p2.String()
	if str2[0] == 'X' {
		t.Error("NewPassword did not make defensive copy")
	}
}

func TestNewPasswordFromString(t *testing.T) {
	passwordStr := "my-secure-password"
	p := NewPasswordFromString(passwordStr)

	if p == nil {
		t.Fatal("NewPasswordFromString returned nil")
	}
...
func TestClearPassword_String(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			p := NewPasswordFromString(tt.password)
			got, err := p.String()
			if err != nil {
				t.Errorf("String() error = %v", err)
				return
...
func TestClearPassword_Interface(t *testing.T) {
	...
	_ = NewPassword([]byte("test"))
	_ = NewPasswordFromString("test")
}
...
func TestPassword_Lifecycle(t *testing.T) {
	...
	// Create
	p := NewPasswordFromString(originalPassword)
...
func TestPassword_EdgeCases(t *testing.T) {
	...
	t.Run("Nil bytes should not panic", func(t *testing.T) {
		defer func() {
			if r := recover(); r != nil {
				t.Errorf("NewPassword(nil) panicked: %v", r)
			}
		}()

		p := NewPassword(nil)
		if p == nil {
			t.Error("NewPassword(nil) returned nil")
		}
