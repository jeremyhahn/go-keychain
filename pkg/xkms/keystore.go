...
// Backend provides a unified interface for cryptographic key and certificate management.
//
// It composes a key provider (for key operations) with certificate storage (always external),
...
type Backend interface {
	// ========================================================================
	// Key Operations (delegated to key provider)
	// ========================================================================
	...
	// The Key ID is parsed to determine the key provider and key name.
	// Returns the key wrapped in an OpaqueKey for safe operations.
	//
	// Format: backend:keyname
	//
	// Example:
	//   key, err := backend.GetKeyByID("pkcs11:my-signing-key")
	...
	//
	// Example:
	//   signer, err := backend.GetSignerByID("tpm2:attestation-key")
	...
	//
	// Example:
	//   decrypter, err := backend.GetDecrypterByID("awskms:rsa-key")
...
// BackendConfig provides configuration for creating a new Backend instance.
type BackendConfig struct {
	// KeyProvider provides key generation and cryptographic operations.
	// Required.
	...
	// key providers like PKCS#11 or TPM2 that support internal cert storage.
	CertStorage storage.Backend
}

// New creates a new Backend instance with the provided configuration.
//
...
// compositeBackend is the internal implementation of Backend.
// It composes a key provider (for keys) with certificate storage (always external).
