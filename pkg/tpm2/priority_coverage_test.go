...
func TestCreateRSAPublicArea(t *testing.T) {
	...
	rsaPublicArea := tpm2.TPMTPublic{
		...
		Unique: tpm2.NewTPMUPublicID(
			...
			&tpm2.TPM2BPublicKeyRSA{
				Buffer: privateKey.PublicKey.N.Bytes(),
			},
		),
	}
	...
	require.NoError(t, err)
	assert.Equal(t, privateKey.PublicKey.N.Bytes(), rsaUnique.Buffer)
}
...
func TestCreateECCPublicArea(t *testing.T) {
	...
	eccPublicArea := tpm2.TPMTPublic{
		...
		Unique: tpm2.NewTPMUPublicID(
			tpm2.TPMAlgECC,
			&tpm2.TPMSECCPoint{
				X: tpm2.TPM2BECCParameter{Buffer: privateKey.PublicKey.X.Bytes()},
				Y: tpm2.TPM2BECCParameter{Buffer: privateKey.PublicKey.Y.Bytes()},
			},
		),
	}
	...
	require.NoError(t, err)
	assert.Equal(t, privateKey.PublicKey.X.Bytes(), eccUnique.X.Buffer)
	assert.Equal(t, privateKey.PublicKey.Y.Bytes(), eccUnique.Y.Buffer)
}
...
func TestBigIntToBytes(t *testing.T) {
	...
	assert.Equal(t, privateKey.PublicKey.X, xReconstructed)
	assert.Equal(t, privateKey.PublicKey.Y, yReconstructed)
}
