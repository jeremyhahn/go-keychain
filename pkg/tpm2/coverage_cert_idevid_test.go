...
// TestWriteIDevIDCertificate_NotConfigured tests error when IDevID not configured
func TestWriteIDevIDCertificate_NotConfigured(t *testing.T) {
...
// TestReadIDevIDCertificate_NotConfigured tests error when IDevID not configured
func TestReadIDevIDCertificate_NotConfigured(t *testing.T) {
	_, tpm := createSim(false, false)
	defer func() { _ = tpm.Close() }()
...
// TestWriteIAKCertificate_NotConfigured tests error when IAK not configured
func TestWriteIAKCertificate_NotConfigured(t *testing.T) {
	_, tpm := createSim(false, false)
	defer func() { _ = tpm.Close() }()
...
// TestReadIAKCertificate_NotConfigured tests error when IAK not configured
func TestReadIAKCertificate_NotConfigured(t *testing.T) {
	_, tpm := createSim(false, false)
	defer func() { _ = tpm.Close() }()
...
// TestReadIDevIDCertificate_NotFound tests certificate not found error
func TestReadIDevIDCertificate_NotFound(t *testing.T) {
	_, tpm := createSim(false, false)
	defer func() { _ = tpm.Close() }()
...
// TestReadIAKCertificate_NotFound tests certificate not found error
func TestReadIAKCertificate_NotFound(t *testing.T) {
	_, tpm := createSim(false, false)
	defer func() { _ = tpm.Close() }()
...
// TestWriteIDevIDCertificate_PublicKeyMismatch tests public key validation
func TestWriteIDevIDCertificate_PublicKeyMismatch(t *testing.T) {
	_, tpm := createSim(false, false)
	defer func() { _ = tpm.Close() }()
...
// TestWriteIAKCertificate_PublicKeyMismatch tests public key validation
func TestWriteIAKCertificate_PublicKeyMismatch(t *testing.T) {
	_, tpm := createSim(false, false)
	defer func() { _ = tpm.Close() }()
...
// TestReadIDevIDCertificate_FromStoreSuccess tests reading cert from store
func TestReadIDevIDCertificate_FromStoreSuccess(t *testing.T) {
	rsaKey, err := rsa.GenerateKey(rand.Reader, 2048)
	require.NoError(t, err)
	...
	// Now read it back
	readCert, err := tpmObj.ReadIDevIDCertificate()
	assert.NoError(t, err)
	assert.NotNil(t, readCert)
	assert.Equal(t, cert.Subject.CommonName, readCert.Subject.CommonName)
}

// TestReadIAKCertificate_FromStoreSuccess tests reading IAK cert from store
func TestReadIAKCertificate_FromStoreSuccess(t *testing.T) {
	rsaKey, err := rsa.GenerateKey(rand.Reader, 2048)
	require.NoError(t, err)
	...
	// Now read it back
	readCert, err := tpmObj.ReadIAKCertificate()
	assert.NoError(t, err)
	assert.NotNil(t, readCert)
	assert.Equal(t, cert.Subject.CommonName, readCert.Subject.CommonName)
}
...
// TestReadIDevIDCertificate_StoreNotFoundError tests specific not found error
func TestReadIDevIDCertificate_StoreNotFoundError(t *testing.T) {
	logger := slog.Default()
	certStore := store.NewMemoryCertStore(logger)
	...
	_, err := tpmObj.ReadIDevIDCertificate()
	assert.ErrorIs(t, err, ErrIDevIDCertNotFound)
}

// TestReadIAKCertificate_StoreNotFoundError tests specific not found error
func TestReadIAKCertificate_StoreNotFoundError(t *testing.T) {
	logger := slog.Default()
	certStore := store.NewMemoryCertStore(logger)
	...
	// Read without storing should return not found
	_, err := tpmObj.ReadIAKCertificate()
