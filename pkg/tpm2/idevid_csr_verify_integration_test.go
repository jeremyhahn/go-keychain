...
func createSelfSignedEKCert(t *testing.T, pub *rsa.PublicKey) *x509.Certificate {
	t.Helper()

	// Generate a temporary key for signing (in real world, this would be the TPM manufacturer's key)
	...
	certBytes, err := x509.CreateCertificate(rand.Reader, template, template, pub, caKey)
	require.NoError(t, err)
