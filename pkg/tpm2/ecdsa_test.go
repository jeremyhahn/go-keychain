...
func TestECCPointEncodingUnit(t *testing.T) {
	...
	// Reconstruct the public key via ParseUncompressedPublicKey
	reconstructed, err := ecdsa.ParseUncompressedPublicKey(elliptic.P256(), uncompressed)
	if err != nil {
		t.Fatalf("failed to reconstruct public key: %v", err)
	}

	if !reconstructed.Equal(&privateKey.PublicKey) {
