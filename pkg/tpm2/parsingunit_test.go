...
func TestParsing_EventLog_UnknownAlgoSkipped(t *testing.T) {
	...
	binary.Write(&buf, binary.LittleEndian, uint16(0x2001))
...
func TestParsing_EventLog_MultiDigest(t *testing.T) {
	...
	binary.Write(&buf, binary.LittleEndian, uint32(0x0004))
	binary.Write(&buf, binary.LittleEndian, uint32(3))
	binary.Write(&buf, binary.LittleEndian, uint16(AlgSHA1))
	buf.Write(make([]byte, 20))
	binary.Write(&buf, binary.LittleEndian, uint16(AlgSHA256))
	buf.Write(make([]byte, 32))
	binary.Write(&buf, binary.LittleEndian, uint16(AlgSHA384))
