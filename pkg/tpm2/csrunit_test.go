...
func TestCSR_UnmarshalIDevIDCSR_Valid(t *testing.T) {
	...
	binary.Write(&buf, binary.BigEndian, uint32(0x00000100))
	binary.Write(&buf, binary.BigEndian, uint32(0x00000040))
	binary.Write(&buf, binary.BigEndian, uint32(0x00000020))

	binary.Write(&buf, binary.BigEndian, uint32(0x00000100))
	binary.Write(&buf, binary.BigEndian, uint32(0x0000000B))
	binary.Write(&buf, binary.BigEndian, uint32(0x00000020))
	binary.Write(&buf, binary.BigEndian, uint32(0x00000004))
...
func TestCSR_PackIDevIDContent_AllHashAlgos(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			binary.Read(reader, binary.BigEndian, &structVer)
			binary.Read(reader, binary.BigEndian, &hashAlgoId)
