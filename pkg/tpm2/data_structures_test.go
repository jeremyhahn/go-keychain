	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func() []byte {
				...
				binary.Write(&buf, binary.BigEndian, [4]byte{0x00, 0x00, 0x00, 0x10}) // Contents
				binary.Write(&buf, binary.BigEndian, [4]byte{0x00, 0x00, 0x00, 0x04}) // SigSz
				// CSR Contents
				binary.Write(&buf, binary.BigEndian, [4]byte{0x00, 0x00, 0x01, 0x00}) // StructVer
				...
				for i := 0; i < 13; i++ {
					binary.Write(&buf, binary.BigEndian, [4]byte{})
				}
				// Signature
				binary.Write(&buf, binary.BigEndian, []byte{1, 2, 3, 4})
				return buf.Bytes()
			},
			wantErr: false,
		},
		{
			...
			setup: func() []byte {
				var buf bytes.Buffer
				binary.Write(&buf, binary.BigEndian, [4]byte{0x00, 0x00, 0x01, 0x00})
				return buf.Bytes()
			},
			wantErr: true,
		},
...
func TestUnpackIDevIDContentUnit(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			setup: func() *bytes.Reader {
				...
				binary.Write(&buf, binary.BigEndian, [4]byte{0x00, 0x00, 0x00, 0x0b}) // HashAlgoId
				binary.Write(&buf, binary.BigEndian, [4]byte{0x00, 0x00, 0x00, 0x20}) // HashSz
				binary.Write(&buf, binary.BigEndian, [4]byte{0x00, 0x00, 0x00, 0x04}) // ProdModelSz
				binary.Write(&buf, binary.BigEndian, [4]byte{0x00, 0x00, 0x00, 0x03}) // ProdSerialSz
				binary.Write(&buf, binary.BigEndian, [4]byte{})                       // ProdCaDataSz
				binary.Write(&buf, binary.BigEndian, [4]byte{})                       // BootEvntLogSz
				binary.Write(&buf, binary.BigEndian, [4]byte{})                       // EkCertSZ
				binary.Write(&buf, binary.BigEndian, [4]byte{})                       // AttestPubSZ
				binary.Write(&buf, binary.BigEndian, [4]byte{})                       // AtCreateTktSZ
				binary.Write(&buf, binary.BigEndian, [4]byte{})                       // AtCertifyInfoSZ
				binary.Write(&buf, binary.BigEndian, [4]byte{})                       // AtCertifyInfoSignatureSZ
				binary.Write(&buf, binary.BigEndian, [4]byte{})                       // SigningPubSZ
				binary.Write(&buf, binary.BigEndian, [4]byte{})                       // SgnCertifyInfoSZ
				binary.Write(&buf, binary.BigEndian, [4]byte{})                       // SgnCertifyInfoSignatureSZ
				binary.Write(&buf, binary.BigEndian, [4]byte{})                       // PadSz
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result == nil {
				t.Error("UnpackIDevIDContent() returned nil")
			}
