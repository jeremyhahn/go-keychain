...
func TestCalculatePCRs(t *testing.T) {
	...
	t.Run("skip unsupported algorithms", func(t *testing.T) {
		...
		if len(result["sha1"]) != 0 || len(result["sha384"]) != 0 || len(result["sha512"]) != 0 {
			// These should remain empty since no events have them
		}
	})
...
func BenchmarkExtendPCR_SHA256(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		ExtendPCR(currentPCR, digest, "sha256")
	}
}
...
func BenchmarkGetHashFunction(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		GetHashFunction(algorithms[i%len(algorithms)])
	}
}
