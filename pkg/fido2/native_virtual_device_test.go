...
func TestNativeVirtualDevice_Create(t *testing.T) {
	...
	if device.Manufacturer() != "go-keychain" {
		t.Errorf("unexpected manufacturer: %s", device.Manufacturer())
	}

	if device.Product() != "NativeFIDO" {
...
func TestNativeVirtualDevice_Init(t *testing.T) {
	...
	binary.BigEndian.PutUint32(packet[0:4], CIDBroadcast)
	...
	if n != HIDPacketSize {
		t.Errorf("Read returned %d, want %d", n, HIDPacketSize)
	}

	// Verify response
	cmd := response[4]
	if cmd != CTAPHID_INIT {
		t.Errorf("expected INIT response (0x%02X), got 0x%02X", CTAPHID_INIT, cmd)
	}
...
func TestNativeVirtualDevice_MakeCredential(t *testing.T) {
	...
	// This is a minimal valid request
	...
	// Map with required fields: clientDataHash, rp, user, pubKeyCredParams
	cborData := buildMakeCredentialCBOR(clientDataHash, "example.com", "Example", []byte{1, 2, 3, 4}, "testuser", "Test User")

	payload := append([]byte{CmdMakeCredential}, cborData...)
	...
	// Check status - either OK or error (depends on user presence simulation)
	// For now we just verify we get a valid response
	if len(response) < 1 {
...
func TestNativeVirtualDevice_GetAssertion(t *testing.T) {
	...
	sendCBORCommand(t, device, cid, append([]byte{CmdGetAssertion}, gaPayload...))
...
func buildMakeCredentialCBOR(clientDataHash []byte, rpID, rpName string, userID []byte, userName, displayName string) []byte {
	// Minimal CBOR map structure (not using full CBOR library for simplicity)
	// {1: clientDataHash, 2: {id: rpID, name: rpName}, 3: {id: userID, name: userName, displayName: displayName}, 4: [{type: "public-key", alg: -7}]}

	// For testing, we'll build a proper CBOR structure
	// Using manual CBOR encoding for predictable test data

	// Simple approach: use the authenticator's CBOR encoder through a test helper
