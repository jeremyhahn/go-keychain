...
// Tests for VirtualEnumerator Open with non-prefix path - covers virtual_enumerator.go:117-119
func TestVirtualEnumerator_OpenByExactPath_FC(t *testing.T) {
	...
	// Open using the exact path
	opened, err := enum.Open(device.Path())
	require.NoError(t, err)
	assert.Equal(t, device.Path(), opened.Path())
}

// Tests for VirtualEnumerator with duplicate registration - covers virtual_enumerator.go:50-51
func TestVirtualEnumerator_RegisterDuplicate_FC(t *testing.T) {
	enum := NewVirtualDeviceEnumerator()

	device, _ := NewVirtualFIDO2Device(&VirtualDeviceConfig{FIDOClient: NewMockFIDOClient(), SerialNumber: "001"})
...
// Tests for VirtualEnumerator register nil device - covers virtual_enumerator.go:43-44
func TestVirtualEnumerator_RegisterNilDevice_FC(t *testing.T) {
	enum := NewVirtualDeviceEnumerator()
...
// Tests for VirtualEnumerator unregister non-existent - covers virtual_enumerator.go:63-64
func TestVirtualEnumerator_UnregisterNonExistent_FC(t *testing.T) {
	enum := NewVirtualDeviceEnumerator()

	err := enum.UnregisterDevice("/nonexistent")
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "not found")
}

// Tests for CombinedEnumerator with enumerate error - covers virtual_enumerator.go:169-173
func TestCombinedEnumerator_EnumerateWithError_FC(t *testing.T) {
	// First enumerator fails
	failingEnum := &FailingEnumeratorFC{}

	// Second enumerator succeeds
	successEnum := NewMockHIDDeviceEnumerator()
	successEnum.AddDevice(NewMockHIDDevice("/dev/hidraw0"))

	combined := NewCombinedEnumerator(failingEnum, successEnum)

	devices, err := combined.Enumerate(0, 0)
	require.NoError(t, err)
	assert.Len(t, devices, 1) // Should still return device from second enumerator
}

// FailingEnumeratorFC always fails
type FailingEnumeratorFC struct{}

func (e *FailingEnumeratorFC) Enumerate(vendorID, productID uint16) ([]HIDDevice, error) {
	return nil, errors.New("enumeration failed")
}

func (e *FailingEnumeratorFC) Open(path string) (HIDDevice, error) {
	return nil, errors.New("open failed")
}

// Tests for CombinedEnumerator Open fallback - covers virtual_enumerator.go:192-196
func TestCombinedEnumerator_OpenNotFoundInAny_FC(t *testing.T) {
	enum1 := NewMockHIDDeviceEnumerator()
	enum2 := NewMockHIDDeviceEnumerator()

	combined := NewCombinedEnumerator(enum1, enum2)

	_, err := combined.Open("/nonexistent")
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "not found")
}
