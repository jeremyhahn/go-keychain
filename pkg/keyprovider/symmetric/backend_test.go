...
func TestDeleteKey(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("DeleteKey(context.Background(), ) failed: %v", err)
	}
...
func TestListKeys(t *testing.T) {
	...
	keys, err := b.ListKeys()
	if err != nil {
		t.Fatalf("ListKeys(context.Background(), ) failed: %v", err)
	}
	if len(keys) != 0 {
...
func TestAsymmetricOperationsNotSupported(t *testing.T) {
	...
	if err == nil {
		t.Error("GetKey(context.Background(), ) should return error for software backend")
	}
...
func (m *MockFailingStorage) Get(_ context.Context, key string) ([]byte, error) {
	return nil, fmt.Errorf("storage error: get failed")
}

func (m *MockFailingStorage) Put(_ context.Context, key string, value []byte) error {
	if m.failOnPut {
...
func (m *MockFailingStorage) Delete(_ context.Context, key string) error {
	if m.failOnDelete {
...
func (m *MockFailingStorage) List(_ context.Context, prefix string) ([]string, error) {
	return nil, fmt.Errorf("storage error: list failed")
}

func (m *MockFailingStorage) Exists(_ context.Context, key string) (bool, error) {
	if m.failOnExists {
...
func (m *MockFailingStorage) Scan(_ context.Context, prefix string, fn func(key string, value []byte) error) error {
	return fmt.Errorf("storage error: scan failed")
}
...
func TestStoreSymmetricKey_WithEmptyPassword(t *testing.T) {
	...
	retrieved, err := storage.GetKey(context.Background(), symmetricBackend.storage, keyID)
	if err != nil {
		t.Fatalf("GetKey(context.Background(), ) failed: %v", err)
	}
...
func (m *MockFailingStorageClose) Get(_ context.Context, key string) ([]byte, error) {
	return nil, storage.ErrNotFound
}

func (m *MockFailingStorageClose) Put(_ context.Context, key string, value []byte) error {
	return nil
}

func (m *MockFailingStorageClose) Delete(_ context.Context, key string) error {
	return nil
}

func (m *MockFailingStorageClose) List(_ context.Context, prefix string) ([]string, error) {
	return []string{}, nil
}

func (m *MockFailingStorageClose) Exists(_ context.Context, key string) (bool, error) {
	return false, nil
}
...
func (m *MockFailingStorageClose) Scan(_ context.Context, prefix string, fn func(key string, value []byte) error) error {
	return fmt.Errorf("storage error: scan failed")
}
...
func TestListKeys_WithMalformedKeyIDs(t *testing.T) {
	...
	if err := storage.SaveKey(context.Background(), s, malformedKeyID, keyData); err != nil {
		t.Fatal(err)
	}

	// Manually insert a key with non-secret keytype (should be skipped)
	asymmetricKeyID := "sw:rsa:test-rsa:rsa-2048"
	if err := storage.SaveKey(context.Background(), s, asymmetricKeyID, keyData); err != nil {
		t.Fatal(err)
	}
...
func TestListKeys_WithInvalidMetadata(t *testing.T) {
	...
	if err := s.Put(context.Background(), metaKey, invalidJSON); err != nil {
		t.Fatal(err)
	}
...
func TestStoreSymmetricKey_WithEmptyPasswordBytes(t *testing.T) {
	...
	storedData, err := storage.GetKey(context.Background(), s, keyID)
	if err != nil {
...
// TestDeleteKey_InvalidAttributes tests DeleteKey with attributes that fail
// Validate (empty CN). This exercises the Validate error path in DeleteKey.
func TestDeleteKey_InvalidAttributes(t *testing.T) {
	s := storage.New()
	config := &Config{KeyStorage: s}
	b, err := NewBackend(config)
	if err != nil {
	...
	// Attributes with empty CN fail Validate
	attrs := &types.KeyAttributes{
		KeyType:            backend.KEY_TYPE_SECRET,
		StoreType:          backend.STORE_SW,
		SymmetricAlgorithm: types.SymmetricAES256GCM,
	...
	err = b.DeleteKey(attrs)
	if err == nil {
		t.Fatal("expected error for DeleteKey with invalid attributes")
	}
	if !errors.Is(err, backend.ErrInvalidAttributes) {
...
// TestExportKey_InvalidAttributesValidation tests ExportKey with Exportable=true
// but attributes that fail Validate (empty CN). This exercises the Validate
// error path inside ExportKey, which is reached after the Exportable check passes.
func TestExportKey_InvalidAttributesValidation(t *testing.T) {
	s := storage.New()
	config := &Config{KeyStorage: s}
	b, err := NewBackend(config)
	if err != nil {
	...
	// Attributes with Exportable=true but empty CN (fails Validate)
	attrs := &types.KeyAttributes{
		Exportable:         true,
		KeyType:            backend.KEY_TYPE_SECRET,
		StoreType:          backend.STORE_SW,
	...
	_, err = symmetricBackend.ExportKey(attrs, backend.WrappingAlgorithmRSAES_OAEP_SHA_256)
	if err == nil {
		t.Fatal("expected error for ExportKey with invalid attributes")
	}
	if !errors.Is(err, backend.ErrInvalidAttributes) {
		t.Errorf("expected ErrInvalidAttributes, got: %v", err)
	}
...
// TestImportKey_EmptyCNValidation tests ImportKey with attributes that fail
// Validate (empty CN). This exercises the Validate error path in ImportKey.
func TestImportKey_EmptyCNValidation(t *testing.T) {
	s := storage.New()
	config := &Config{KeyStorage: s}
	b, err := NewBackend(config)
	if err != nil {
	...
	// Attributes with empty CN (fails Validate)
	attrs := &types.KeyAttributes{
		KeyType:            backend.KEY_TYPE_SECRET,
		StoreType:          backend.STORE_SW,
		SymmetricAlgorithm: types.SymmetricAES256GCM,
	...
	err = symmetricBackend.ImportKey(attrs, &backend.WrappedKeyMaterial{
		WrappedKey:  []byte("wrapped"),
		Algorithm:   backend.WrappingAlgorithmRSAES_OAEP_SHA_256,
		ImportToken: []byte("token-id"),
	})
	if err == nil {
		t.Fatal("expected error for ImportKey with invalid attributes")
	}
	if !errors.Is(err, backend.ErrInvalidAttributes) {
		t.Errorf("expected ErrInvalidAttributes, got: %v", err)
	}
...
// TestExportKey_WrongPassword tests ExportKey with a password-protected key
// where the export is attempted with an incorrect password. This exercises
// the decodeSymmetricKey error path in ExportKey.
func TestExportKey_WrongPassword(t *testing.T) {
	s := storage.New()
	config := &Config{KeyStorage: s}
	b, err := NewBackend(config)
	if err != nil {
	...
	wrongPwAttrs := *attrs
	wrongPwAttrs.Password = backend.StaticPassword([]byte("wrong-password"))

	_, err = symmetricBackend.ExportKey(&wrongPwAttrs, backend.WrappingAlgorithmRSAES_OAEP_SHA_256)
	if err == nil {
		t.Fatal("expected error for ExportKey with wrong password")
	}
...
// TestGenerateSymmetricKey_NonSymmetricEd25519 tests GenerateSymmetricKey with
// Ed25519 attributes (valid asymmetric attrs that pass Validate but IsSymmetric
// returns false). This exercises the !attrs.IsSymmetric() error path.
func TestGenerateSymmetricKey_NonSymmetricEd25519(t *testing.T) {
	s := storage.New()
	config := &Config{KeyStorage: s}
	b, err := NewBackend(config)
	if err != nil {
	...
	_, err = b.GenerateSymmetricKey(attrs)
	if err == nil {
		t.Fatal("expected error for GenerateSymmetricKey with Ed25519 attributes")
	}
