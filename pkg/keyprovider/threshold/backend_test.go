...
func TestThresholdBackend_GenerateRSAKey(t *testing.T) {
	...
	exists, err := keyStorage.Exists(context.Background(), metadataKey)
	require.NoError(t, err)
...
func TestThresholdBackend_GenerateECDSAKey(t *testing.T) {
	...
	require.True(t, ok)
	// Verify the private key is complete (has both private scalar and public key)
	assert.NotNil(t, ecdsaKey.Public(), "ECDSA key should have a public component")
	_, err = x509.MarshalECPrivateKey(ecdsaKey)
	assert.NoError(t, err, "ECDSA key should be marshalable (private scalar present)")
}
