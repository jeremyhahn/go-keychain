...
func TestThresholdSigner_Public_InsufficientShares(t *testing.T) {
	...
	for i := 1; i <= 3; i++ {
		...
		_ = keyStorage.Delete(context.Background(), shareKey)
...
func TestThresholdBackend_ListKeys_CorruptedMetadata(t *testing.T) {
	...
	_ = keyStorage.Put(context.Background(), "threshold/metadata/corrupted-key", []byte("invalid json"))
...
func TestThresholdBackend_loadShare_CorruptedJSON(t *testing.T) {
	...
	keyID := backend.getKeyID(attrs)
	shareKey := "threshold/shares/" + keyID + "/share-1"
	_ = keyStorage.Put(context.Background(), shareKey, []byte("corrupted json"))
...
func TestThresholdBackend_loadAnyShare_CorruptedShares(t *testing.T) {
	...
	keyID := backend.getKeyID(attrs)
	_ = keyStorage.Put(context.Background(), "threshold/shares/"+keyID+"/share-1", []byte("bad"))
	_ = keyStorage.Put(context.Background(), "threshold/shares/"+keyID+"/share-2", []byte("bad"))
...
func TestThresholdBackend_ListKeys_UnreadableEntry(t *testing.T) {
	...
	err = keyStorage.Put(context.Background(), "threshold/metadata/bad-json-key", []byte("not json"))
	require.NoError(t, err)
...
func TestThresholdBackend_loadShare_InvalidShare(t *testing.T) {
	...
	err = keyStorage.Put(context.Background(), shareKey, []byte(invalidShare))
	require.NoError(t, err)
...
func TestThresholdSigner_reconstructKey_SkipsCorruptedShares(t *testing.T) {
	...
	keyID := backend.getKeyID(attrs)
	shareKey := "threshold/shares/" + keyID + "/share-2"
	err = keyStorage.Put(context.Background(), shareKey, []byte("corrupted"))
	require.NoError(t, err)
