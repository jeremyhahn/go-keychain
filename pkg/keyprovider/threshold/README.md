...
- **ML-DSA-44** - NIST PQC Level 2 security (via Cloudflare circl)
- **ML-DSA-65** - NIST PQC Level 3 security (via Cloudflare circl)
- **ML-DSA-87** - NIST PQC Level 5 security (via Cloudflare circl)
...
```go
import (
    "github.com/jeremyhahn/go-xkms/pkg/keyprovider/threshold"
...
Quantum support is always available -- no special build tags or flags are required.

```go
// Generate a 3-of-5 threshold ML-DSA-44 key
attrs := &types.KeyAttributes{
    CN:        "quantum-ca",
    KeyType:   types.KeyTypeSigning,
    StoreType: types.StoreThreshold,
...
Quantum support (ML-DSA) is always compiled in. No special build tags, environment variables, or external libraries are required.
...
- **Quantum Key Reconstruction**: Simplified implementation for ML-DSA seed-based key splitting
...
### Run All Tests (including quantum)
```bash
...
go test ./pkg/backend/threshold/ -v -run TestThresholdSigner_Quantum
```
