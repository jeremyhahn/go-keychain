...
func Example_standardNames() {
	...
	fmt.Println(MetricKeyGenerate) // xkms.key.generate
	fmt.Println(MetricKeyImport)   // xkms.key.import
	fmt.Println(MetricKeyExport)   // xkms.key.export
	fmt.Println(MetricSign)        // xkms.crypto.sign
	fmt.Println(MetricErrorTotal)  // xkms.error.total
	fmt.Println(MetricLatencySign) // xkms.latency.crypto.sign
