...
func TestWriteJSON(t *testing.T) {
	...
	t.Run("EncodingError", func(t *testing.T) {
		...
		// Status code is set before encoding, so it will be OK
		// The error path just logs but doesn't change status
		if w.Code != http.StatusOK {
			t.Errorf("Expected status %d, got %d", http.StatusOK, w.Code)
		}
	})
...
// faultyResponseWriter is a writer that fails on Write to simulate encoding errors
...
func TestWriteError_EncodingFailure(t *testing.T) {
	// This test exercises the encoding error path in writeError
	// We use a faulty writer that fails on Write
	w := newFaultyResponseWriter()
	err := errors.New("test error")
	...
	if w.statusCode != http.StatusBadRequest {
		t.Errorf("Expected status %d, got %d", http.StatusBadRequest, w.statusCode)
	}
}

func TestWriteErrorWithMessage_EncodingFailure(t *testing.T) {
	// This test exercises the encoding error path in writeErrorWithMessage
	w := newFaultyResponseWriter()
	err := errors.New("test error")

	// This should not panic even if encoding fails
	writeErrorWithMessage(w, err, "custom message", http.StatusInternalServerError)
