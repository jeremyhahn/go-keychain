...
// newBufconnTransport creates an in-memory gRPC server + connected Transport for testing.
...
func TestSignTCGCSR_Connected(t *testing.T) {
	...
	require.NoError(t, err)
	assert.NotEmpty(t, resp.IAKCertDER)
}
...
func TestErrNotSupported_Methods(t *testing.T) {
	...
	tests := []struct {
		...
		{"GetInitStatus", func() error { _, err := tr.GetInitStatus(context.Background()); return err }},
		{"ClaimCertBegin", func() error { _, err := tr.ClaimCertBegin(context.Background(), &transport.ClaimCertBeginRequest{}); return err }},
		{"ClaimCertComplete", func() error { _, err := tr.ClaimCertComplete(context.Background(), &transport.ClaimCertCompleteRequest{}); return err }},
		{"ClaimShare", func() error { _, err := tr.ClaimShare(context.Background(), &transport.ClaimShareRequest{}); return err }},
		{"SubmitCredential", func() error { _, err := tr.SubmitCredential(context.Background(), &transport.CredentialSubmitRequest{}); return err }},
		{"GetCredentialStrategy", func() error { _, err := tr.GetCredentialStrategy(context.Background()); return err }},
	...
	t.Run("SignCSRInit", func(t *testing.T) {
...
func TestConvertPasswordEntryToTransport_Nil(t *testing.T) {
	assert.Nil(t, convertPasswordEntryToTransport(nil))
}

func TestConvertPasswordEntryToTransport_WithTimestamps(t *testing.T) {
	now := time.Now()
	entry := &pb.PasswordEntry{
		Id: "pw-1", Name: "test", Username: "user",
		ExpiresAt: timestamppb.New(now.Add(time.Hour)),
		CreatedAt: timestamppb.New(now),
		UpdatedAt: timestamppb.New(now),
	}
	result := convertPasswordEntryToTransport(entry)
...
func TestConvertJSONToBytes_Empty(t *testing.T) {
	assert.Nil(t, ConvertJSONToBytes(nil))
}

func TestConvertJSONToBytes_ValidBase64(t *testing.T) {
	assert.Equal(t, []byte("test"), ConvertJSONToBytes([]byte(`"dGVzdA=="`)))
}

func TestConvertJSONToBytes_InvalidJSON(t *testing.T) {
	raw := []byte(`not-json`)
	assert.Equal(t, raw, ConvertJSONToBytes(raw))
}
