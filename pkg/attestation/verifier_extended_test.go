...
func TestVerifySignatureNilAttestingKey(t *testing.T) {
	...
	if !errors.Is(err, ErrNilAttestingKey) {
		t.Errorf("expected ErrNilAttestingKey, got: %v", err)
	}
}
...
func TestVerifySignatureUnsupportedKeyType(t *testing.T) {
	// Use a custom type that is not RSA, ECDSA, or Ed25519
	type unsupportedPublicKey struct{}

	stmt := &AttestationStatement{
		Format:                "tpm2",
		AttestingKeyAlgorithm: x509.RSA,
		AttestingKeyPublic:    unsupportedPublicKey{},
		Signature:             []byte("test-sig"),
		SignatureAlgorithm:    x509.SHA256WithRSA,
		AttestationData:       []byte("test data"),
		AttestedKeyPublic:     &rsa.PublicKey{},
...
func TestVerifyRSASignatureUnsupportedAlgorithm(t *testing.T) {
	...
	if !errors.Is(err, ErrUnsupportedSignatureAlgorithm) {
		t.Errorf("expected ErrUnsupportedSignatureAlgorithm, got: %v", err)
	}
}
...
func TestVerifyPCRsExpectedButNoneProvided(t *testing.T) {
	...
	if !errors.Is(err, ErrMissingPCRs) {
		t.Errorf("expected ErrMissingPCRs, got: %v", err)
	}
}
...
func TestVerifyPCRsMissingPCR(t *testing.T) {
	...
	if !errors.Is(err, ErrPCRMissing) {
		t.Errorf("expected ErrPCRMissing, got: %v", err)
	}
}
...
func TestHashInvalidPublicKey(t *testing.T) {
	...
	if !errors.Is(err, ErrMarshalAttestedKey) {
		t.Errorf("expected ErrMarshalAttestedKey, got: %v", err)
	}
}
...
func TestVerifyRSASignatureHashNil(t *testing.T) {
	...
	// Use an algorithm that would create a nil hash (not in the map)
...
// TestVerifyECDSASignatureSHA256 tests ECDSA signature verification with SHA256
func TestVerifyECDSASignatureSHA256(t *testing.T) {
	privKey, err := ecdsa.GenerateKey(elliptic.P256(), rand.Reader)
	if err != nil {
		t.Fatalf("Failed to generate ECDSA key: %v", err)
	}

	data := []byte("test attestation data for ECDSA")
	h := sha256.Sum256(data)
	...
	stmt := &AttestationStatement{
		...
		AttestingKeyPublic:    &privKey.PublicKey,
		Signature:             sig,
		SignatureAlgorithm:    x509.ECDSAWithSHA256,
		AttestationData:       data,
		AttestedKeyPublic:     &privKey.PublicKey,
	}
	...
	err = v.VerifySignature(stmt, opts)
	if err != nil {
		t.Errorf("VerifySignature() failed with ECDSA SHA256: %v", err)
	}
}

// TestVerifyECDSASignatureSHA384 tests ECDSA signature verification with SHA384
func TestVerifyECDSASignatureSHA384(t *testing.T) {
	privKey, err := ecdsa.GenerateKey(elliptic.P384(), rand.Reader)
	if err != nil {
		t.Fatalf("Failed to generate ECDSA key: %v", err)
	}

	data := []byte("test attestation data for ECDSA-384")
	h := sha512.Sum384(data)
	...
	err = v.VerifySignature(stmt, opts)
	if err != nil {
		t.Errorf("VerifySignature() failed with ECDSA SHA384: %v", err)
	}
}

// TestVerifyECDSASignatureSHA512 tests ECDSA signature verification with SHA512
func TestVerifyECDSASignatureSHA512(t *testing.T) {
	privKey, err := ecdsa.GenerateKey(elliptic.P521(), rand.Reader)
	if err != nil {
		t.Fatalf("Failed to generate ECDSA key: %v", err)
	}

	data := []byte("test attestation data for ECDSA-512")
	h := sha512.Sum512(data)
	...
	err = v.VerifySignature(stmt, opts)
	if err != nil {
		t.Errorf("VerifySignature() failed with ECDSA SHA512: %v", err)
	}
}

// TestVerifyECDSASignatureInvalid tests ECDSA signature verification with invalid signature
func TestVerifyECDSASignatureInvalid(t *testing.T) {
	...
	stmt := &AttestationStatement{
		...
		AttestingKeyAlgorithm: x509.ECDSA,
		AttestingKeyPublic:    &privKey.PublicKey,
		Signature:             []byte("invalid-ecdsa-signature"),
		SignatureAlgorithm:    x509.ECDSAWithSHA256,
		AttestationData:       []byte("test data"),
		AttestedKeyPublic:     &privKey.PublicKey,
	}
	...
	err = v.VerifySignature(stmt, opts)
	if err == nil {
		t.Errorf("VerifySignature() should fail with invalid ECDSA signature")
	}
...
// TestVerifyECDSASignatureUnsupportedAlgorithm tests ECDSA with unsupported signature algorithm
func TestVerifyECDSASignatureUnsupportedAlgorithm(t *testing.T) {
	privKey, err := ecdsa.GenerateKey(elliptic.P256(), rand.Reader)
	if err != nil {
		t.Fatalf("Failed to generate ECDSA key: %v", err)
	}
	...
	stmt := &AttestationStatement{
		...
		AttestingKeyPublic:    &privKey.PublicKey,
		Signature:             []byte("test-sig"),
		SignatureAlgorithm:    x509.SHA256WithRSA, // Wrong algorithm for ECDSA
		AttestationData:       []byte("test data"),
		AttestedKeyPublic:     &privKey.PublicKey,
	}
	...
	err = v.VerifySignature(stmt, opts)
	if err == nil {
		t.Errorf("VerifySignature() should fail with unsupported algorithm for ECDSA")
	}
	if !errors.Is(err, ErrUnsupportedSignatureAlgorithm) {
		t.Errorf("expected ErrUnsupportedSignatureAlgorithm, got: %v", err)
	}
...
func TestVerifyEd25519Signature(t *testing.T) {
	...
	data := []byte("test attestation data for Ed25519")

	// Ed25519 signs raw data (no pre-hashing)
	sig := ed25519.Sign(privKey, data)

	stmt := &AttestationStatement{
		Format:                "tpm2",
		AttestingKeyAlgorithm: x509.Ed25519,
		AttestingKeyPublic:    pubKey,
		Signature:             sig,
	...
	err = v.VerifySignature(stmt, opts)
	if err != nil {
		t.Errorf("VerifySignature() failed with Ed25519: %v", err)
	}
}

// TestVerifyEd25519SignatureInvalid tests Ed25519 with invalid signature
func TestVerifyEd25519SignatureInvalid(t *testing.T) {
	...
	stmt := &AttestationStatement{
		...
		AttestingKeyAlgorithm: x509.Ed25519,
		AttestingKeyPublic:    pubKey,
		Signature:             []byte("invalid-ed25519-signature-that-is-long-enough-for-the-check-to-proceed"),
		SignatureAlgorithm:    x509.PureEd25519,
		AttestationData:       []byte("test data"),
		AttestedKeyPublic:     pubKey,
	}
	...
	err = v.VerifySignature(stmt, opts)
	if err == nil {
		t.Errorf("VerifySignature() should fail with invalid Ed25519 signature")
	}
	if !errors.Is(err, ErrSignatureVerificationFailed) {
		t.Errorf("expected ErrSignatureVerificationFailed, got: %v", err)
	}
...
// TestVerifyEd25519SignatureWrongKey tests Ed25519 with data signed by different key
func TestVerifyEd25519SignatureWrongKey(t *testing.T) {
	...
	sig := ed25519.Sign(privKey2, data)

	stmt := &AttestationStatement{
		Format:                "tpm2",
		AttestingKeyAlgorithm: x509.Ed25519,
		AttestingKeyPublic:    pubKey1, // Different key
		Signature:             sig,
		SignatureAlgorithm:    x509.PureEd25519,
		AttestationData:       data,
		AttestedKeyPublic:     pubKey1,
	}
	...
	err = v.VerifySignature(stmt, opts)
	if err == nil {
		t.Errorf("VerifySignature() should fail when Ed25519 signature is from wrong key")
	}
	if !errors.Is(err, ErrSignatureVerificationFailed) {
		t.Errorf("expected ErrSignatureVerificationFailed, got: %v", err)
	}
...
// TestVerifyChainSelfSignedECDSA tests chain verification with ECDSA self-signed cert
func TestVerifyChainSelfSignedECDSA(t *testing.T) {
	privKey, err := ecdsa.GenerateKey(elliptic.P256(), rand.Reader)
	if err != nil {
		t.Fatalf("Failed to generate ECDSA key: %v", err)
	}
	...
	err = v.VerifyChain(stmt, opts)
	if err != nil {
		t.Errorf("VerifyChain() failed with ECDSA self-signed cert: %v", err)
	}
}

// TestVerifyNonceMismatchTypedError tests that nonce mismatch returns typed error
func TestVerifyNonceMismatchTypedError(t *testing.T) {
	stmt := &AttestationStatement{
...
// TestVerifyEmptyCertificateChainTypedError tests typed error for empty chain
func TestVerifyEmptyCertificateChainTypedError(t *testing.T) {
	stmt := &AttestationStatement{
	...
	err := v.VerifyChain(stmt, opts)
	if !errors.Is(err, ErrEmptyCertificateChain) {
		t.Errorf("expected ErrEmptyCertificateChain, got: %v", err)
	}
}

// BenchmarkVerifyECDSASignature benchmarks ECDSA signature verification
func BenchmarkVerifyECDSASignature(b *testing.B) {
	privKey, _ := ecdsa.GenerateKey(elliptic.P256(), rand.Reader)

	data := []byte("test attestation data")
	h := sha256.Sum256(data)
	sig, _ := ecdsa.SignASN1(rand.Reader, privKey, h[:])
...
// BenchmarkVerifyEd25519Signature benchmarks Ed25519 signature verification
func BenchmarkVerifyEd25519Signature(b *testing.B) {
