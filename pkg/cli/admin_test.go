...
func TestResolveStoragePath_WithEnvVar(t *testing.T) {
	originalEnv := os.Getenv("KEYCHAIN_STORAGE_PATH")
	defer func() {
		if originalEnv != "" {
			if err := os.Setenv("KEYCHAIN_STORAGE_PATH", originalEnv); err != nil {
	...
	if err := os.Setenv("KEYCHAIN_STORAGE_PATH", testPath); err != nil {
...
func TestResolveStoragePath_Default(t *testing.T) {
	...
	if result != "/var/lib/keychain" {
		t.Errorf("resolveStoragePath() = %v, want /var/lib/keychain", result)
	}
}
