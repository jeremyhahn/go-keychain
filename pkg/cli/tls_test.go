...
func TestGetTLSCertLocal_Success_RSA_TextOutput(t *testing.T) {
	...
	if err := certStorage.SaveCert("test-tls-key", ca.Cert); err != nil {
...
func TestGetTLSCertLocal_Success_ECDSA_JSONOutput(t *testing.T) {
	...
	certStorage := storage.NewCertAdapter(storageBackend)
	if err := certStorage.SaveCert("test-ecdsa-tls", ca.Cert); err != nil {
		t.Fatalf("failed to save certificate: %v", err)
	}
...
func TestGetTLSCertLocal_Success_WithCertChain(t *testing.T) {
	...
	certStorage := storage.NewCertAdapter(storageBackend)
	if err := certStorage.SaveCert("test-chain-key", serverCert.Cert); err != nil {
		t.Fatalf("failed to save certificate: %v", err)
	}
	// Save the CA as the chain
	chain := []*x509.Certificate{ca.Cert}
	if err := certStorage.SaveCertChain("test-chain-key", chain); err != nil {
...
func TestGetTLSCertLocal_TableOutput(t *testing.T) {
	...
	certStorage := storage.NewCertAdapter(storageBackend)
	if err := certStorage.SaveCert("table-output-key", ca.Cert); err != nil {
		t.Fatalf("failed to save certificate: %v", err)
	}
...
func TestGetTLSCertLocal_VerboseMode(t *testing.T) {
	...
	certStorage := storage.NewCertAdapter(storageBackend)
	if err := certStorage.SaveCert("verbose-key", ca.Cert); err != nil {
		t.Fatalf("failed to save certificate: %v", err)
	}
...
func TestGetTLSCertLocal_CertChainNotFound_ContinuesSuccessfully(t *testing.T) {
	...
	certStorage := storage.NewCertAdapter(storageBackend)
	if err := certStorage.SaveCert("no-chain-key", ca.Cert); err != nil {
		t.Fatalf("failed to save certificate: %v", err)
	}
...
func TestGetTLSCertLocal_JSONOutput_WithChain(t *testing.T) {
	...
	certStorage := storage.NewCertAdapter(storageBackend)
	if err := certStorage.SaveCert("json-chain-key", serverCert.Cert); err != nil {
		t.Fatalf("failed to save certificate: %v", err)
	}
	chain := []*x509.Certificate{ca.Cert}
	if err := certStorage.SaveCertChain("json-chain-key", chain); err != nil {
		t.Fatalf("failed to save certificate chain: %v", err)
	}
...
func TestGetTLSCertLocal_Success_SigningKeyType(t *testing.T) {
	...
	certStorage := storage.NewCertAdapter(storageBackend)
	if err := certStorage.SaveCert("signing-key", ca.Cert); err != nil {
		t.Fatalf("failed to save certificate: %v", err)
	}
...
func TestGetTLSCertLocal_Success_EncryptionKeyType(t *testing.T) {
	...
	certStorage := storage.NewCertAdapter(storageBackend)
	if err := certStorage.SaveCert("encryption-key", ca.Cert); err != nil {
		t.Fatalf("failed to save certificate: %v", err)
	}
...
func TestGetTLSCertLocal_Ed25519_KeyNotFound(t *testing.T) {
	tmpDir := t.TempDir()
	...
	// Create mock backend without Ed25519 key
	mockBackend := mocks.NewExtendedMockBackend()
	...
	certStorage := storage.NewCertAdapter(storageBackend)
	if err := certStorage.SaveCert("ed25519-key", ca.Cert); err != nil {
		t.Fatalf("failed to save certificate: %v", err)
	}
	...
	exitCode := captureExit(t, func() {
		getTLSCertLocal(cfg, printer, "ed25519-key", "signing", "ed25519", 0, "")
	})

	// Should fail because key doesn't exist
	if exitCode != 1 {
		t.Errorf("expected exit code 1 for non-existent key, got %d", exitCode)
	}
}

func TestGetTLSCertLocal_Success_4096BitRSA(t *testing.T) {
	tmpDir := t.TempDir()
	...
	mockBackend := mocks.NewExtendedMockBackend()
	mockBackend.StoreKey("rsa-4096-key", rsaKey)
	...
	certStorage := storage.NewCertAdapter(storageBackend)
	if err := certStorage.SaveCert("rsa-4096-key", ca.Cert); err != nil {
		t.Fatalf("failed to save certificate: %v", err)
	}
	...
	exitCode := captureExit(t, func() {
		getTLSCertLocal(cfg, printer, "rsa-4096-key", "tls", "rsa", 4096, "")
	})
...
func TestGetTLSCertLocal_Success_P521Curve(t *testing.T) {
	tmpDir := t.TempDir()
	...
	mockBackend := mocks.NewExtendedMockBackend()
	mockBackend.StoreKey("p521-key", ecKey)
	...
	certStorage := storage.NewCertAdapter(storageBackend)
	if err := certStorage.SaveCert("p521-key", ca.Cert); err != nil {
		t.Fatalf("failed to save certificate: %v", err)
	}
	...
	exitCode := captureExit(t, func() {
		getTLSCertLocal(cfg, printer, "p521-key", "tls", "ecdsa", 0, "P-521")
	})
...
func TestGetTLSCertLocal_Error_PrintTLSCertificateUnknownFormat(t *testing.T) {
	tmpDir := t.TempDir()
	...
	mockBackend := mocks.NewExtendedMockBackend()
	mockBackend.StoreKey("unknown-format-key", rsaKey)

	cfg := NewConfig()
	cfg.KeyDir = tmpDir
	cfg.OutputFormat = "text" // Use text for global config
	cfg.UseLocal = true
	cfg.BackendFactory = func(c *Config) (types.Backend, error) {
		return mockBackend, nil
	}
	...
	certStorage := storage.NewCertAdapter(storageBackend)
	if err := certStorage.SaveCert("unknown-format-key", ca.Cert); err != nil {
		t.Fatalf("failed to save certificate: %v", err)
	}
	...
	// Use a printer with an unknown format that will trigger the default case error
	printer := NewPrinter("unknown-format", io.Discard)

	exitCode := captureExit(t, func() {
		getTLSCertLocal(cfg, printer, "unknown-format-key", "tls", "rsa", 2048, "P-256")
	})

	// PrintTLSCertificate returns error for unknown format
	if exitCode != 1 {
		t.Errorf("expected exit code 1 for unknown format error, got %d", exitCode)
	}
}
...
func TestGetTLSCertLocal_JSONWriteError(t *testing.T) {
	tmpDir := t.TempDir()
	...
	mockBackend := mocks.NewExtendedMockBackend()
	mockBackend.StoreKey("json-write-error-key", rsaKey)
	...
	certStorage := storage.NewCertAdapter(storageBackend)
	if err := certStorage.SaveCert("json-write-error-key", ca.Cert); err != nil {
		t.Fatalf("failed to save certificate: %v", err)
	}
	...
	// Use a printer with an error-prone writer for JSON format
	// JSON encoder writes to the writer and should propagate errors
	printer := NewPrinter("json", &ErrMockWriter{})

	exitCode := captureExit(t, func() {
		getTLSCertLocal(cfg, printer, "json-write-error-key", "tls", "rsa", 2048, "P-256")
	})

	// JSON encoder should fail when writing to the error writer
	if exitCode != 1 {
		t.Errorf("expected exit code 1 for JSON write error, got %d", exitCode)
	}
}
