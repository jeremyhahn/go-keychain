...
// YubiKey supports sealing via envelope encryption using its PIV RSA keys.
...
// Seal encrypts/protects data using envelope encryption with YubiKey's RSA key.
//
// The envelope encryption process:
//  1. Generate a random Data Encryption Key (DEK) using YubiKey's hardware RNG
//  2. Encrypt the plaintext data with AES-256-GCM using the DEK
//  3. Encrypt the DEK with the YubiKey's RSA public key (RSA-OAEP)
//  4. Store the encrypted DEK in the sealed data metadata
//
// Security characteristics:
//   - DEK is generated using YubiKey's FIPS-certified hardware RNG
//   - DEK is protected by YubiKey's RSA key (cannot be decrypted without YubiKey)
//   - Data encryption happens in software (AES-256-GCM)
//   - YubiKey never sees the plaintext data (only encrypts/decrypts the DEK)
//
...
func (b *Backend) Seal(ctx context.Context, data []byte, opts *types.SealOptions) (*types.SealedData, error) {
	...
	// Step 1: Generate random DEK using YubiKey's hardware RNG
	...
	if err != nil {
		return nil, fmt.Errorf("yubikey: failed to get sealing key: %w", err)
	}
	...
	case crypto.Signer:
		publicKey = k.Public()
	default:
		return nil, fmt.Errorf("yubikey: key does not implement crypto.Signer")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("yubikey: failed to encrypt DEK: %w", err)
	}
	...
	if aad != nil {
		sealed.Metadata["yubikey:aad_hash"] = hashBytes(aad)
	}

	return sealed, nil
}

// Unseal decrypts/recovers data using envelope encryption with YubiKey's RSA key.
//
// The envelope decryption process:
//  1. Extract the encrypted DEK from the sealed data metadata
//  2. Decrypt the DEK using the YubiKey's RSA private key (RSA-OAEP)
//  3. Decrypt the ciphertext using AES-256-GCM with the recovered DEK
//
// This operation requires:
//   - The YubiKey device with the private key
//   - Correct PIN for YubiKey access
...
func (b *Backend) Unseal(ctx context.Context, sealed *types.SealedData, opts *types.UnsealOptions) ([]byte, error) {
	...
	if !ok {
		return nil, ErrEncryptedDEKNotFound
	}
	...
	// Step 2: Get the YubiKey's crypto.Decrypter to decrypt the DEK
	decrypter, err := b.pkcs11.Decrypter(keyAttrs)
	if err != nil {
		return nil, fmt.Errorf("yubikey: failed to get decrypter: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("yubikey: failed to decrypt DEK (check PIN and key): %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("yubikey: failed to decrypt (check key and AAD): %w", err)
	}
