...
// 1. Generate a random DEK (Data Encryption Key) using YubiKey's hardware RNG
// 2. Encrypt the data with software AES-256-GCM using the DEK
// 3. Encrypt the DEK with the YubiKey's key using algorithm-specific wrapping:
...
// yubikeySymmetricKey implements types.SymmetricKey for YubiKey envelope encryption.
// The actual key material is a software DEK that gets encrypted with a key in a PIV slot.
type yubikeySymmetricKey struct {
	algorithm        string
	keySize          int
...
func (k *yubikeySymmetricKey) Algorithm() string {
	return k.algorithm
}

// KeySize returns the key size in bits.
func (k *yubikeySymmetricKey) KeySize() int {
	return k.keySize
}

// Raw returns an error because YubiKey-backed symmetric keys use envelope encryption
// and don't expose the raw DEK material - each encryption generates a new DEK.
func (k *yubikeySymmetricKey) Raw() ([]byte, error) {
	return nil, fmt.Errorf("%w: YubiKey symmetric keys use envelope encryption and don't expose raw key material", backend.ErrNotSupported)
}

// yubikeySymmetricEncrypter implements types.SymmetricEncrypter using envelope encryption.
// All encryption/decryption operations use a randomly generated DEK that is wrapped with
// the YubiKey's key using the appropriate algorithm.
type yubikeySymmetricEncrypter struct {
	backend          *Backend
...
// 1. Generate random 32-byte DEK via YubiKey's hardware RNG
// 2. Encrypt plaintext with AES-256-GCM using the DEK
// 3. Encrypt DEK with YubiKey's key using appropriate algorithm (RSA/ECIES/X25519)
// 4. Return encrypted data with encrypted DEK and algorithm in metadata
func (e *yubikeySymmetricEncrypter) Encrypt(plaintext []byte, opts *types.EncryptOptions) (*types.EncryptedData, error) {
	if opts == nil {
		opts = &types.EncryptOptions{}
	}

	// STEP 1: Generate random 32-byte DEK using YubiKey's hardware RNG
	dek, err := e.backend.GenerateRandom(dekSize)
	if err != nil {
		return nil, fmt.Errorf("yubikey: failed to generate DEK: %w", err)
	}
	defer func() {
	...
	if err != nil {
		return nil, fmt.Errorf("yubikey: failed to create AES cipher: %w", err)
	}

	gcm, err := cipher.NewGCM(block)
	if err != nil {
		return nil, fmt.Errorf("yubikey: failed to create GCM: %w", err)
	}
	...
	if opts.Nonce != nil {
		...
		if _, err := rand.Read(nonce); err != nil {
			return nil, fmt.Errorf("yubikey: failed to generate nonce: %w", err)
		}
	}
	...
	// STEP 3: Encrypt DEK with YubiKey's key using appropriate algorithm
	// Get the wrapping key's decrypter to access the public key
	decrypter, err := e.backend.Decrypter(e.wrappingKeyAttrs)
	if err != nil {
		return nil, fmt.Errorf("yubikey: failed to get wrapping key decrypter: %w", err)
	}
	...
	case *ecdsa.PublicKey:
		...
		default:
			return nil, fmt.Errorf("%w: unsupported ECDSA curve: %s", backend.ErrInvalidAlgorithm, pub.Curve.Params().Name)
		}
	...
	default:
		return nil, fmt.Errorf("%w: wrapping key type %T not supported (must be RSA, ECDSA, or Ed25519)", ErrUnsupportedAlgorithm, pubKey)
	}

	if err != nil {
		return nil, fmt.Errorf("yubikey: failed to wrap DEK: %w", err)
	}
...
// 2. Decrypt DEK using YubiKey's key with appropriate algorithm
// 3. Decrypt ciphertext with AES-256-GCM using the DEK
func (e *yubikeySymmetricEncrypter) Decrypt(data *types.EncryptedData, opts *types.DecryptOptions) ([]byte, error) {
	if opts == nil {
	...
	if data.Algorithm != "aes256-gcm-envelope" {
		return nil, fmt.Errorf("%w: %s (expected aes256-gcm-envelope)", ErrUnsupportedEncryptionAlgorithm, data.Algorithm)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("yubikey: failed to decode encrypted DEK: %w", err)
	}
	...
	// STEP 2: Decrypt DEK using YubiKey's key with appropriate algorithm
	decrypter, err := e.backend.Decrypter(e.wrappingKeyAttrs)
	if err != nil {
		return nil, fmt.Errorf("yubikey: failed to get wrapping key decrypter: %w", err)
	}
	...
	default:
		return nil, fmt.Errorf("%w: unknown wrap algorithm: %s", ErrUnsupportedAlgorithm, wrapAlgo)
	}

	if err != nil {
		return nil, fmt.Errorf("yubikey: failed to unwrap DEK: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("yubikey: decryption failed (authentication error): %w", err)
	}
...
func wrapDEKWithRSA(pubKey *rsa.PublicKey, dek []byte) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("yubikey: RSA-OAEP encryption failed: %w", err)
	}
...
func unwrapDEKWithRSA(decrypter crypto.Decrypter, encryptedDEK []byte) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("yubikey: RSA-OAEP decryption failed: %w", err)
	}
	return dekBytes, nil
}
...
func wrapDEKWithECIES(pubKey *ecdsa.PublicKey, dek []byte) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("yubikey: failed to generate ephemeral key: %w", err)
	}
	...
	if _, err := io.ReadFull(kdf, derivedKey); err != nil {
		return nil, fmt.Errorf("yubikey: HKDF failed: %w", err)
	}
	defer func() {
...
func unwrapDEKWithECIES(decrypter crypto.Decrypter, encryptedDEK []byte, curve elliptic.Curve) ([]byte, error) {
	...
	if len(ciphertextAndTag) < 16 {
		return nil, ErrCiphertextAndTagTooShort
	}
	...
	if ephemeralX == nil {
		return nil, ErrUnmarshalPublicKeyFailed
	}
	...
	if !ok {
		return nil, fmt.Errorf("%w: got %T", ErrNotECDSA, privKey.PrivateKey())
	}
	...
	if err != nil {
		return nil, fmt.Errorf("yubikey: GCM decryption failed: %w", err)
	}
...
func wrapDEKWithX25519(edPubKey ed25519.PublicKey, dek []byte) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("yubikey: failed to convert Ed25519 to X25519: %w", err)
	}

	// Generate ephemeral X25519 key pair
	ephemeralPub, ephemeralPriv, err := generateX25519KeyPair()
	if err != nil {
		return nil, fmt.Errorf("yubikey: failed to generate ephemeral X25519 key: %w", err)
	}
	defer func() {
	...
	if err != nil {
		return nil, fmt.Errorf("yubikey: X25519 key agreement failed: %w", err)
	}
...
func unwrapDEKWithX25519(decrypter crypto.Decrypter, encryptedDEK []byte) ([]byte, error) {
	...
	if !ok {
		return nil, fmt.Errorf("%w: got %T", ErrNotEd25519, pubKey)
	}
	...
	if !ok {
		return nil, ErrDecrypterNoEd25519PrivateKey
	}

	edPrivKey, ok := privKeyProvider.PrivateKey().(ed25519.PrivateKey)
	if !ok {
		return nil, fmt.Errorf("%w: got %T", ErrNotEd25519, privKeyProvider.PrivateKey())
	}

	// Convert Ed25519 private key to X25519
	x25519PrivKey, err := ed25519PrivateKeyToX25519(edPrivKey)
	if err != nil {
		return nil, fmt.Errorf("yubikey: failed to convert Ed25519 private key to X25519: %w", err)
	}
	defer func() {
...
func ed25519PublicKeyToX25519(edPub ed25519.PublicKey) ([]byte, error) {
	if len(edPub) != ed25519.PublicKeySize {
		return nil, fmt.Errorf("%w: got %d, expected %d", ErrInvalidEd25519PublicKeySize, len(edPub), ed25519.PublicKeySize)
	}
...
func ed25519PrivateKeyToX25519(edPriv ed25519.PrivateKey) ([]byte, error) {
	if len(edPriv) != ed25519.PrivateKeySize {
		return nil, fmt.Errorf("%w: got %d, expected %d", ErrInvalidEd25519PrivateKeySize, len(edPriv), ed25519.PrivateKeySize)
	}

	// Ed25519 private key format is 64 bytes: 32-byte seed || 32-byte public key
	// The X25519 private key is derived from the seed using SHA512
	seed := edPriv[:32]

	// Hash the seed with SHA512 and clamp the first 32 bytes
...
func generateX25519KeyPair() (publicKey, privateKey []byte, err error) {
	...
	if _, err := rand.Read(privateKey); err != nil {
		return nil, nil, fmt.Errorf("yubikey: failed to generate random scalar: %w", err)
	}
	...
	if err != nil {
		return nil, nil, fmt.Errorf("yubikey: failed to compute public key: %w", err)
	}
...
func (b *Backend) GenerateSymmetricKey(attrs *types.KeyAttributes) (types.SymmetricKey, error) {
	...
	if err := attrs.Validate(); err != nil {
		return nil, fmt.Errorf("yubikey: invalid attributes: %w", err)
	}
	...
	if attrs.WrapAttributes == nil {
		return nil, fmt.Errorf("%w: WrapAttributes required for YubiKey symmetric keys (must reference PIV slot key)", backend.ErrInvalidAttributes)
	}
	...
	case *ecdsa.PublicKey:
		...
		if curveName != "P-256" && curveName != "P-384" {
			return nil, fmt.Errorf("%w: ECDSA curve %s not supported (must be P-256 or P-384)", backend.ErrInvalidAlgorithm, curveName)
		}
	case ed25519.PublicKey:
		// Check firmware version
		if !b.SupportsX25519() {
			return nil, fmt.Errorf("%w: have %d.%d", ErrFirmwareRequired, b.firmwareVer.Major, b.firmwareVer.Minor)
		}
	default:
...
func (b *Backend) GetSymmetricKey(attrs *types.KeyAttributes) (types.SymmetricKey, error) {
	...
	// Validate that wrapping key attributes are provided
	if attrs.WrapAttributes == nil {
		return nil, fmt.Errorf("%w: WrapAttributes required for YubiKey symmetric keys", backend.ErrInvalidAttributes)
	}
...
func (b *Backend) SymmetricEncrypter(attrs *types.KeyAttributes) (types.SymmetricEncrypter, error) {
	...
	// Type assert to get the YubiKey-specific key
	yubikeyKey, ok := key.(*yubikeySymmetricKey)
	if !ok {
		return nil, fmt.Errorf("%w: got %T", ErrUnexpectedKeyType, key)
	}

	return &yubikeySymmetricEncrypter{
		backend:          b,
		attrs:            attrs,
		wrappingKeyAttrs: yubikeyKey.wrappingKeyAttrs,
	}, nil
}
