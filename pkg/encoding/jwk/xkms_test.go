...
// Mock xkms getter functions for testing
var (
...
// TestFromXKMS tests creating a JWK from a xkms key.
func TestFromXKMS(t *testing.T) {
	tests := []struct {
		name        string
		keyID       string
		getKeyFunc  XKMSKeyGetter
		expectError bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			jwk, err := FromXKMS(tt.keyID, tt.getKeyFunc)
...
// TestLoadKeyFromXKMS tests loading a key from xkms using JWK kid.
func TestLoadKeyFromXKMS(t *testing.T) {
	tests := []struct {
		name        string
		jwk         *JWK
		getKeyFunc  XKMSKeyGetter
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			key, err := tt.jwk.LoadKeyFromXKMS(tt.getKeyFunc)
...
// TestIsXKMSBacked tests the IsXKMSBacked method.
func TestIsXKMSBacked(t *testing.T) {
	tests := []struct {
		name     string
		jwk      *JWK
		expected bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := tt.jwk.IsXKMSBacked()
			assert.Equal(t, tt.expected, result)
		})
	}
}

// TestToXKMSSigner tests getting a signer from a xkms-backed JWK.
func TestToXKMSSigner(t *testing.T) {
	tests := []struct {
		name           string
		jwk            *JWK
		getSignerFunc  XKMSSignerGetter
		expectError    bool
		expectedSigner crypto.Signer
	}{
		{
			name: "valid xkms-backed JWK",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			signer, err := tt.jwk.ToXKMSSigner(tt.getSignerFunc)
...
// TestJWK_RoundTrip tests creating a JWK from xkms and loading it back.
func TestJWK_RoundTrip(t *testing.T) {
	keyID := "pkcs8:roundtrip-test"

	// Create JWK from xkms
	jwk, err := FromXKMS(keyID, func(kid string) (crypto.PrivateKey, error) {
		return testRSAKey, nil
	})
	require.NoError(t, err)
	require.NotNil(t, jwk)
	...
	assert.True(t, jwk.IsXKMSBacked())
	...
	assert.True(t, jwk2.IsXKMSBacked())

	// Load key from deserialized JWK
	key, err := jwk2.LoadKeyFromXKMS(func(kid string) (crypto.PrivateKey, error) {
		if kid == keyID {
			return testRSAKey, nil
		}
		return nil, errKeyNotFound
...
// TestJWK_NoPrivateKeyMaterial ensures xkms JWKs don't contain private key material.
func TestJWK_NoPrivateKeyMaterial(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			jwk, err := FromXKMS(tt.keyID, func(kid string) (crypto.PrivateKey, error) {
				return tt.key, nil
			})
			require.NoError(t, err)
