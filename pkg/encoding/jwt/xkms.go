...
// XKMSSigner signs JWTs using keys from a xkms.
...
type XKMSSigner struct {
	getKey    jwk.XKMSKeyGetter
	getSigner jwk.XKMSSignerGetter
}

// NewXKMSSigner creates a new xkms-based JWT signer.
// The getKey and getSigner functions should retrieve keys from your xkms.
//
// Example:
//
...
func NewXKMSSigner(getKey jwk.XKMSKeyGetter, getSigner jwk.XKMSSignerGetter) *XKMSSigner {
	return &XKMSSigner{
		getKey:    getKey,
		getSigner: getSigner,
	}
...
// SignWithKeyID signs a JWT using a key from the xkms identified by Key ID.
...
func (ks *XKMSSigner) SignWithKeyID(keyID string, claims jwt.Claims) (string, error) {
	// Get the signer from xkms
...
func (ks *XKMSSigner) SignWithKeyIDAndAlgorithm(keyID string, claims jwt.Claims, alg Algorithm) (string, error) {
	// Get the signer from xkms
	signer, err := ks.getSigner(keyID)
	if err != nil {
		return "", fmt.Errorf("failed to get signer for key %s: %w", keyID, err)
...
// XKMSVerifier verifies JWTs using keys from a xkms or JWKs
type XKMSVerifier struct {
	getKey jwk.XKMSKeyGetter
}

// NewXKMSVerifier creates a new xkms-based JWT verifier.
//
// Example:
//
...
//	    return keystore.GetKeyByID(keyID)
//	})
func NewXKMSVerifier(getKey jwk.XKMSKeyGetter) *XKMSVerifier {
	return &XKMSVerifier{
		getKey: getKey,
	}
}

// VerifyWithKeyID verifies a JWT using a public key from the xkms.
// The Key ID should match the kid in the JWT header.
//
// Example:
//
//	token, err := verifier.VerifyWithKeyID(tokenString, "pkcs11:signing-key")
func (kv *XKMSVerifier) VerifyWithKeyID(tokenString, keyID string) (*jwt.Token, error) {
	// Get the key from xkms
...
// and using it to retrieve the verification key from the xkms.
//
// Example:
//
//	token, err := verifier.VerifyWithAutoKeyID(tokenString)
func (kv *XKMSVerifier) VerifyWithAutoKeyID(tokenString string) (*jwt.Token, error) {
...
func (kv *XKMSVerifier) VerifyWithJWK(tokenString string, key *jwk.JWK) (*jwt.Token, error) {
