...
let currentTOTP: XKeyTOTPResult | null = null;
...
async function init(): Promise<void> {
  ...
  let statusResp: { type: string; data?: XKeyStatusResult; required?: boolean; message?: string };
  try {
  ...
  try {
    ...
    if (searchResp.type === 'credentials') {
      renderCredentials(searchResp.data as XKeyCredential[]);
  ...
  try {
    ...
    if (totpResp.type === 'totp_result' && totpResp.data) {
      showTOTP(totpResp.data as XKeyTOTPResult);
    }
  } catch {
...
function updateStatus(resp: { type: string; data?: XKeyStatusResult; message?: string }): void {
...
function renderCredentials(credentials: XKeyCredential[]): void {
...
async function handleFill(cred: XKeyCredential): Promise<void> {
  try {
...
function showTOTP(totp: XKeyTOTPResult): void {
  currentTOTP = totp;
 ...
 */
function showPairingSection(): void {
  pairingActive = true;
  ...
  if (credentialsList) credentialsList.style.display = 'none';
 ...
 */
function hidePairingSection(): void {
  pairingActive = false;
  ...
  if (credentialsList) credentialsList.style.display = 'block';
}
 ...
 */
async function handlePairingSubmit(): Promise<void> {
  const codeInput = document.getElementById('pairing-code') as HTMLInputElement | null;
  const errorEl = document.getElementById('pairing-error');
  const successEl = document.getElementById('pairing-success');
  const submitBtn = document.getElementById('pairing-submit') as HTMLButtonElement | null;
 ...
 */
async function handlePairingRetry(): Promise<void> {
  const retryBtn = document.getElementById('pairing-retry');
  const errorEl = document.getElementById('pairing-error');
  const codeInput = document.getElementById('pairing-code') as HTMLInputElement | null;
 ...
 */
function showUnlockSection(): void {
 ...
 */
function hideUnlockSection(): void {
  const unlockSection = document.getElementById('unlock-section');
  const credentialsList = document.getElementById('credentials-list');
 ...
 */
async function handleUnlockSubmit(): Promise<void> {
  const pinInput = document.getElementById('unlock-pin') as HTMLInputElement | null;
  const errorEl = document.getElementById('unlock-error');
  const submitBtn = document.getElementById('unlock-submit') as HTMLButtonElement | null;
...
function showConnectButton(): void {
  ...
  if (document.getElementById('connect-btn')) return;
...
async function handleConnect(): Promise<void> {
  ...
  try {
    const resp = await chrome.runtime.sendMessage({ type: 'reconnect' }) as {
      type: string;
      connected?: boolean;
      locked?: boolean;
      message?: string;
