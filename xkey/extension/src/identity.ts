...
export class ExtensionIdentity {
  ...
  static async getOrCreate(): Promise<{
    ...
    const signingKey = await crypto.subtle.importKey(
      'jwk',
      privateJwk,
      'Ed25519' as unknown as EcKeyImportParams,
      false,
      ['sign'],
    );
    ...
    return {
      ...
      sign: async (data: Uint8Array): Promise<Uint8Array> => {
        const sig = await crypto.subtle.sign(
          'Ed25519' as unknown as EcdsaParams,
          signingKey,
   ...
   */
  static async signHandshake(ephemeralPubKey: Uint8Array, origin: string): Promise<Uint8Array> {
    ...
    const bindingMessage = new Uint8Array(
      ephemeralPubKey.length + originBytes.length + contextBytes.length,
    );
   ...
   */
  static async getPublicKey(): Promise<Uint8Array | null> {
   ...
   */
  private static async loadFromStorage(): Promise<{
    publicKey: Uint8Array;
    sign: (data: Uint8Array) => Promise<Uint8Array>;
  } | null> {
    const result = await chrome.storage.local.get(STORAGE_KEY);
    ...
    try {
      ...
      return {
        publicKey: publicKeyBytes,
        sign: async (rawData: Uint8Array): Promise<Uint8Array> => {
          const sig = await crypto.subtle.sign(
            'Ed25519' as unknown as EcdsaParams,
            signingKey,
