 ...
 */
async function deriveKey(sharedSecret, info) {
...
export class SessionCrypto {
     ...
     */
    async completeHandshake(peerPubKeyBase64) {
        const peerPubBytes = fromBase64(peerPubKeyBase64);
        // Import peer's public key for ECDH.
        const peerPubKey = await crypto.subtle.importKey('raw', peerPubBytes.buffer, 'X25519', false, []);
        // Derive the shared secret via ECDH.
        const sharedSecret = await crypto.subtle.deriveBits({ name: 'X25519', public: peerPubKey }, this.privateKey, 256);
    ...
    async encrypt(plaintext) {
        ...
        const ciphertextBuf = await crypto.subtle.encrypt({ name: 'AES-GCM', iv: iv.buffer }, this.sendKey, plaintextBytes);
        return {
            nonce,
     ...
     */
    async decrypt(nonce, ciphertextBase64) {
        if (!this.recvKey) {
            throw new Error('SessionCrypto: handshake not completed');
        }
        // Replay protection: nonces must arrive in strict order.
        if (nonce !== this.recvNonce) {
            throw new Error(`SessionCrypto: unexpected nonce ${nonce}, expected ${this.recvNonce}`);
        }
        this.recvNonce++;
        const iv = buildNonce(nonce);
        const ciphertextBytes = fromBase64(ciphertextBase64);
        const plaintextBuf = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: iv.buffer }, this.recvKey, ciphertextBytes.buffer);
