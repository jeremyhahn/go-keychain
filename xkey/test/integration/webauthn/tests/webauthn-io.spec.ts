...
/**
 * WebAuthn Protocol Integration Tests using Chrome CDP Virtual Authenticators
 *
 * These tests use Chrome DevTools Protocol (CDP) virtual authenticators to test
 * WebAuthn protocol compliance against webauthn.io. They verify that the WebAuthn
 * flow works correctly from a browser perspective.
 *
 * NOTE: These tests do NOT test the actual xkey FIDO2 implementation or its
 * key backends (software, TPM2). For backend-specific tests, see:
 * - test/integration/xkey/backend_test.go (Go-based integration tests)
 *
 * The backend tests exercise the full MakeCredential + GetAssertion flow with
 * actual signature verification for both software and TPM2 backends.
 */

/**
 * Creates a CDP virtual authenticator that mimics xkeyfido2 behavior:
 * - CTAP2 protocol
 * - USB transport (UHID)
