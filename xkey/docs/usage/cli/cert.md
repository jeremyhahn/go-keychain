...
The certificate management commands work with both local storage (using `--local` flag) and remote xkmsd server. All commands support multiple output formats including text, JSON, and YAML.
...
**Usage:**
```bash
xkmsctl cert save <key-id> <cert-file>
```
...
xkmsctl cert save --local my-server /path/to/cert.pem

# Save to remote server
xkmsctl cert save --backend software server-1 ./certificates/server.crt

# Save with verbose output
xkmsctl cert save -v my-cert certificate.pem
```
...
**JSON Output:**
```bash
xkmsctl cert save --output json my-cert cert.pem
```
```json
{
  "success": true,
...
xkmsctl cert get my-server

# Get in JSON format
xkmsctl cert get --output json my-server

# Get from specific backend
xkmsctl cert get --backend tpm2 my-cert
```
...
xkmsctl cert delete my-server

# Delete with confirmation
xkmsctl cert delete --local old-cert

# Delete from specific backend
xkmsctl cert delete --backend pkcs11 expired-cert
```
...
# List in JSON format
xkmsctl cert list --output json

# List from specific backend
xkmsctl cert list --backend tpm2
```
...
xkmsctl cert exists my-server

# Check with JSON output
xkmsctl cert exists --output json my-cert

# Use in scripts
if xkmsctl cert exists my-server; then
...
**Usage:**
```bash
xkmsctl cert save-chain <key-id> <cert-file>...
```
...
xkmsctl cert save-chain server-chain \
  server.crt \
  intermediate-ca.crt \
  root-ca.crt

# Save chain locally
xkmsctl cert save-chain --local my-chain \
  leaf.pem \
  ca.pem

# Save with verbose output to see each certificate
xkmsctl cert save-chain -v tls-chain \
  server.crt \
  intermediate.crt \
  root.crt
```
...
xkmsctl cert get-chain server-chain

# Get in JSON format
xkmsctl cert get-chain --output json my-chain

# Get from specific backend
xkmsctl cert get-chain --backend pkcs11 tls-chain
```
...
**Usage:**
```bash
xkmsctl cert generate-ca [flags]
```
...
**Optional Flags:**
- `--org <string>`: Organization name (default: "Go xKMS")
...
xkmsctl cert generate-ca --cn "My Root CA"

# Generate CA with full details and save to files
xkmsctl cert generate-ca \
  --cn "Example Root CA" \
...
xkmsctl cert generate-ca --cn "JSON CA" --output json
```
...
CA Certificate Details:
  Subject: CN=My Root CA,O=Go xKMS
...
```json
{
  "success": true,
  "subject": "CN=My Root CA,O=Go xKMS",
  "issuer": "CN=My Root CA,O=Go xKMS",
  "serial": "123456789012345678901234567890",
  "not_before": "2024-01-01T00:00:00Z",
  "not_after": "2034-01-01T00:00:00Z",
  "is_ca": true,
...
- `--backend <string>`: Backend to use (default: "software")
...
# 4. Save certificates to xkms
xkmsctl cert save --local ca-cert ca.crt
xkmsctl cert save --local server-cert server.crt
xkmsctl cert save --local client-cert client.crt

# 5. Save certificate chain
xkmsctl cert save-chain --local server-chain server.crt ca.crt

# 6. List all certificates
xkmsctl cert list --local

# 7. Verify certificate exists
xkmsctl cert exists --local server-cert
```
...
```bash
# Check if certificate exists
if xkmsctl cert exists old-server; then
  # Get current certificate to check expiration
  xkmsctl cert get old-server
  ...
  xkmsctl cert save server-cert new-server.crt

  # Delete old certificate
  xkmsctl cert delete old-server
fi
```
...
mkdir -p cert-backup
for cert_id in $(xkmsctl cert list --output json | jq -r '.certificates[]'); do
  xkmsctl cert get "$cert_id" > "cert-backup/${cert_id}.pem"
done

# Restore: Import all certificates
for cert_file in cert-backup/*.pem; do
  cert_id=$(basename "$cert_file" .pem)
  xkmsctl cert save "$cert_id" "$cert_file"
done
```
...
xkmsctl cert save --backend software my-cert cert.pem
xkmsctl cert save --backend tpm2 my-cert cert.pem
xkmsctl cert save --backend pkcs11 my-cert cert.pem

# Verify certificate exists in all backends
xkmsctl cert exists --backend software my-cert
xkmsctl cert exists --backend tpm2 my-cert
xkmsctl cert exists --backend pkcs11 my-cert
...
# Save to xkms
xkmsctl cert save-chain web-server server.crt internal-ca.crt
```
...
# Save to xkms
xkmsctl cert save mtls-server api-server.crt
xkmsctl cert save mtls-client api-client.crt
```
...
for service in auth api gateway storage; do
  ...
  xkmsctl cert save "service-${service}" "${service}.crt"
done
...
# Check if certificate exists
if ! xkmsctl cert exists "$CERT_ID"; then
  echo "Certificate does not exist, creating new one..."
else
  # Get current certificate expiration
  expiry=$(xkmsctl cert get --output json "$CERT_ID" | jq -r '.not_after')
  ...
  xkmsctl cert get "$CERT_ID" > "backup-${CERT_ID}-$(date +%Y%m%d).pem"
fi
...
# Save new certificate
xkmsctl cert save "$CERT_ID" new-cert.crt
