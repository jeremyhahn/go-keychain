...
xkmsctl tls get web-server

# Get TLS certificate with explicit algorithm
xkmsctl tls get api-server --key-algorithm rsa --key-size 4096
```
...
xkmsctl tls get edge-device --key-algorithm ecdsa --curve P-256

# Get ECDSA certificate with P-384 curve
xkmsctl tls get secure-api --key-algorithm ecdsa --curve P-384

# Get ECDSA certificate with P-521 curve
xkmsctl tls get high-security --key-algorithm ecdsa --curve P-521
```
...
xkmsctl tls get app-server --key-algorithm rsa --key-size 2048

# Get RSA certificate with 3072-bit key
xkmsctl tls get legacy-app --key-algorithm rsa --key-size 3072

# Get RSA certificate with 4096-bit key
xkmsctl tls get critical-system --key-algorithm rsa --key-size 4096
```
...
xkmsctl --output json tls get web-server
```
...
xkmsctl tls get web-server
# or explicitly
xkmsctl --output text tls get web-server
```
...
```bash
xkmsctl --output json tls get web-server | jq .
```
...
```bash
xkmsctl tls get web-server > server.pem
```
...
**Example:**
```bash
xkmsctl tls get web-server --key-algorithm rsa --key-size 2048
```
...
**Example:**
```bash
xkmsctl tls get secure-api --key-algorithm ecdsa --curve P-256
```
...
```bash
# Use software backend (default)
xkmsctl --backend software tls get web-server
```
...
xkmsctl --backend tpm2 tls get edge-device --key-algorithm ecdsa --curve P-256

# Use PKCS#11 HSM
xkmsctl --backend pkcs11 tls get secure-server --key-algorithm rsa --key-size 4096
```
...
```bash
# Use AWS KMS
xkmsctl --backend awskms tls get cloud-app

# Use GCP KMS
xkmsctl --backend gcpkms tls get gcp-service

# Use Azure Key Vault
xkmsctl --backend azurekv tls get azure-app
```
...
xkmsctl tls get web-server > /etc/ssl/certs/server.pem
...
xkmsctl tls get api-server --key-algorithm ecdsa --curve P-256

# Verify certificate details
xkmsctl --output json tls get api-server | jq '.certificate'
```
...
xkmsctl tls get lb-frontend --key-algorithm rsa --key-size 2048
```
...
xkmsctl tls get service-a --key-algorithm ecdsa --curve P-256
xkmsctl tls get service-b --key-algorithm ecdsa --curve P-256
```
...
xkmsctl tls get iot-gateway --key-algorithm ecdsa --curve P-256
```
...
xkmsctl --output json tls get web-server | jq '.certificate | {not_before, not_after}'

# Verify DNS names
xkmsctl --output json tls get web-server | jq '.certificate.dns_names'

# Check key algorithm and size
xkmsctl --output json tls get web-server | jq '.certificate | {key_algorithm, key_size}'
```
...
--backend <name>      Backend to use: software, tpm2, pkcs11, etc.
...
```bash
xkmsctl --verbose tls get web-server
```
...
```bash
xkmsctl tls get missing-server
...
```bash
xkmsctl tls get web-server --key-algorithm invalid
...
xkmsctl tls get modern-app --key-algorithm ecdsa --curve P-256

# Legacy compatibility: Use RSA 2048
xkmsctl tls get legacy-app --key-algorithm rsa --key-size 2048

# High security: Use ECDSA P-384 or RSA 4096
xkmsctl tls get critical-app --key-algorithm ecdsa --curve P-384
```
...
xkmsctl cert create web-server-new --key-algorithm ecdsa --curve P-256

# Retrieve and deploy new certificate
xkmsctl tls get web-server-new

# After grace period, rotate to new certificate
xkmsctl cert rotate web-server web-server-new
```
...
xkmsctl tls get web-server > backup/web-server.pem

# For hardware backends, maintain certificate copies in cert storage
xkmsctl cert export web-server > backup/web-server-cert.pem
```
...
```bash
xkmsctl --verbose tls get self-signed
...
xkmsctl --backend pkcs11 --pin "$HSM_PIN" tls get web-server
