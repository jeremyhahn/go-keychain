...
The `user` command group provides comprehensive user account management for the go-xkms system. It supports passwordless authentication using FIDO2 security keys, enabling secure access without the need for traditional passwords.
...
```bash
xkmsctl user register <username> [flags]
```
...
- `--rp-id <id>` - Relying Party ID (default: "go-xkms")
- `--rp-name <name>` - Relying Party name (default: "Go xKMS")
...
- `--storage-path <path>` - Path to xkms storage (default: $XKMS_STORAGE_PATH or /var/lib/xkms)

**Examples:**

Register first admin user:
```bash
xkmsctl user register admin@example.com --display-name "System Administrator"
```

Register operator with specific role:
```bash
xkmsctl user register ops@example.com --role operator --display-name "Operations Team"
```

Register with PIN verification required:
```bash
xkmsctl user register secure@example.com --user-verification --display-name "Security Admin"
```

Register using specific FIDO2 device:
```bash
xkmsctl user register user@example.com --device /dev/hidraw0
```
...
- `--storage-path <path>` - Path to xkms storage

**Examples:**

Login with credential:
```bash
xkmsctl user login --credential-id 'bXVlcnR5dWlvcGFzZGZnaGprbA' --salt 'c2FsdHZhbHVlYXNkZg'
```
...
```bash
xkmsctl user list --storage-path /custom/path
```
...
```bash
xkmsctl user get <username> [flags]
```
...
Get user details:
```bash
xkmsctl user get admin@example.com
```
...
```bash
xkmsctl user delete <username> [flags]
```
...
Delete a user:
```bash
xkmsctl user delete user@example.com
```
...
```bash
xkmsctl user disable <username> [flags]
```
...
Disable a user:
```bash
xkmsctl user disable ops@example.com
```
...
```bash
xkmsctl user enable <username> [flags]
```
...
Enable a user:
```bash
xkmsctl user enable ops@example.com
```
...
No users configured.
Run 'xkmsctl user register <username>' to create the first administrator.
```
...
```bash
xkmsctl user credentials <username> [flags]
```
...
```bash
xkmsctl user credentials admin@example.com
```
...
go-xkms supports the following user roles:
...
- Keep regular backups of the xkms storage directory
...
2. Register first admin:
   ```bash
   xkmsctl user register admin@example.com --display-name "Primary Admin"
   ```

3. Register additional users:
   ```bash
   xkmsctl user register ops@example.com --role operator
   xkmsctl user register audit@example.com --role auditor
   ```
...
6. Login (for scripting/automation):
   ...
   SALT="c2FsdHZhbHVlYXNkZg"
   xkmsctl user login --credential-id "$CRED_ID" --salt "$SALT"
   ```
...
users=$(xkmsctl user list --output json)
echo "$users" | jq '.total'

# Get specific user details
user_info=$(xkmsctl user get admin@example.com --output json)
echo "$user_info" | jq '.role'

# Check if setup is required
status=$(xkmsctl user status --output json)
