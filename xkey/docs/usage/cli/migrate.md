...
**Usage:**
```bash
xkmsctl migrate plan --from <source-backend> --to <dest-backend> [flags]
```

**Required Flags:**
- `--from` - Source backend name (e.g., software, tpm2, pkcs11, awskms, gcpkms, azurekv, vault)
...
```bash
xkmsctl migrate plan --from software --to tpm2 \
  --key-types signing
```

Plan migration for keys matching pattern:
```bash
xkmsctl migrate plan --from awskms --to azurekv \
  --cn-pattern "^prod-.*"
```

Plan migration for recently created keys:
```bash
xkmsctl migrate plan --from gcpkms --to vault \
...
**Output (JSON Format):**
```bash
xkmsctl migrate plan --from software --to pkcs11 --output json
```
```json
{
...
**Usage:**
```bash
xkmsctl migrate execute --from <source-backend> --to <dest-backend> [flags]
```
...
```bash
xkmsctl migrate execute --from software --to pkcs11 --force
```

Migrate and delete source keys after verification:
```bash
xkmsctl migrate execute --from software --to tpm2 \
  --delete-source
```

Parallel migration for better performance:
```bash
xkmsctl migrate execute --from awskms --to azurekv \
  --parallel 4
```

Migrate only signing keys:
```bash
xkmsctl migrate execute --from pkcs11 --to tpm2 \
  --key-types signing
```

Migrate keys matching pattern:
```bash
xkmsctl migrate execute --from gcpkms --to vault \
  --cn-pattern "^staging-.*" \
  --parallel 2 \
  --force
```
...
**Output (JSON Format):**
```bash
xkmsctl migrate execute --from software --to pkcs11 --output json --force
```
```json
{
  "successful_count": 14,
...
**Usage:**
```bash
xkmsctl migrate validate --key-id <key-id> --from <source> --to <dest>
```
...
### Local to Local
- **Software → PKCS#11** - Software to hardware migration
- **Software → TPM2** - Software to TPM chip
- **PKCS#11 → TPM2** - HSM to TPM migration

### Cloud to Local
- **AWS KMS → PKCS#11** - Cloud to hardware
- **GCP KMS → TPM2** - Cloud to TPM
- **Azure Key Vault → Software** - Cloud to software
...
### Local to Cloud
- **Software → AWS KMS** - Software to cloud
- **PKCS#11 → GCP KMS** - Hardware to cloud
- **TPM2 → Azure Key Vault** - TPM to cloud

### Backend Support Matrix

| Source     | Software | PKCS#11 | TPM2 | AWS KMS | GCP KMS | Azure KV | Vault |
|------------|----------|---------|------|---------|---------|----------|-------|
| Software   | ✓        | ✓       | ✓    | ✓       | ✓       | ✓        | ✓     |
| PKCS#11    | ✓      | ✓       | ✓    | ✓       | ✓       | ✓        | ✓     |
| TPM2       | ✓      | ✓       | ✓    | ✓       | ✓       | ✓        | ✓     |
...
Migrate all keys from software to hardware security module (PKCS#11):
...
xkmsctl migrate execute --from awskms --to tpm2 \
  --parallel 4 \
  --force
...
xkmsctl migrate plan --from gcpkms --to azurekv \
  --cn-pattern "^staging-.*"

# 2. Test with staging
xkmsctl migrate execute --from gcpkms --to azurekv \
  --cn-pattern "^staging-.*" \
  --force
...
xkmsctl migrate validate --key-id "critical-key" \
  --from software --to pkcs11
...
xkmsctl backend info --name pkcs11
...
xkmsctl migrate validate --key-id "problem-key" \
  --from software --to pkcs11
```
...
xkmsctl backend status --name pkcs11
...
xkmsctl migrate validate --key-id "failed-key" \
  --from software --to pkcs11
```
...
xkmsctl migrate execute --from pkcs11 --to gcpkms \
  --parallel 4
```
