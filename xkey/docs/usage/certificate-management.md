...
This guide covers certificate storage and management in go-xkms, including hardware-backed certificate storage options for PKCS#11 HSMs and TPM2 devices.

## Overview

go-xkms provides flexible certificate storage options:
...
|----------|-------------|
| `xkms.Certificate(attrs)` | Get a certificate by key attributes |
| `xkms.CertificateByID(kid)` | Get a certificate by key ID string |
| `xkms.SaveCertificate(attrs, cert)` | Save a certificate |
| `xkms.SaveCertificateByID(kid, cert)` | Save a certificate by key ID string |
| `xkms.DeleteCertificate(attrs)` | Delete a certificate |
| `xkms.DeleteCertificateByID(kid)` | Delete a certificate by key ID string |
| `xkms.CertificateChain(attrs)` | Get a certificate chain |
| `xkms.SaveCertificateChain(attrs, chain)` | Save a certificate chain |
| `xkms.CertificateExists(attrs)` | Check if a certificate exists |
| `xkms.CertificateExistsByID(kid)` | Check existence by key ID |
| `xkms.TLSCertificate(attrs)` | Get a `tls.Certificate` for TLS use |
| `xkms.TLSCertificateByID(kid)` | Get a TLS certificate by key ID |
...
func main() {
    ...
    // Retrieve it
    retrieved, err := xkms.Certificate(attrs)
    if err != nil {
        log.Fatal(err)
    }
    fmt.Printf("Certificate subject: %s\n", retrieved.Subject.CommonName)

    // Check existence
    exists, err := xkms.CertificateExists(attrs)
    if err != nil {
        log.Fatal(err)
    }
    fmt.Printf("Certificate exists: %v\n", exists)
...
### String-Based Key ID Operations

For convenience, you can use string key IDs instead of `KeyAttributes`:

```go
// Save using key ID
err := xkms.SaveCertificateByID("software:::api.example.com", cert)

// Retrieve using key ID
cert, err := xkms.CertificateByID("software:::api.example.com")

// Check existence
exists, err := xkms.CertificateExistsByID("software:::api.example.com")

// Delete
err = xkms.DeleteCertificateByID("software:::api.example.com")
```
...
// Save a certificate chain (leaf + intermediates + root)
chain := []*x509.Certificate{leafCert, intermediateCert, rootCert}
err := xkms.SaveCertificateChain(attrs, chain)
if err != nil {
    log.Fatal(err)
}

// Retrieve the chain
retrieved, err := xkms.CertificateChain(attrs)
if err != nil {
    log.Fatal(err)
}
...
err = xkms.SaveCertificateChainByID("software:::api.example.com", chain)
chain, err = xkms.CertificateChainByID("software:::api.example.com")
```

### TLS Certificates

Get a complete `tls.Certificate` ready for use in TLS servers/clients:
...
// Get TLS certificate (includes private key and chain)
tlsCert, err := xkms.TLSCertificate(attrs)
if err != nil {
    log.Fatal(err)
}
...
// Or use the string-based variant
tlsCert, err = xkms.TLSCertificateByID("software:::api.example.com")
```

### Listing Certificates

```go
// List all certificates across all backends
allCerts, err := xkms.ListCertificates()
for _, certID := range allCerts {
    fmt.Println(certID)
}

// List certificates from a specific backend
softwareCerts, err := xkms.ListCertificates("software")
```
...
go-xkms follows a clean separation between keys and certificates:
...
var err error
for i := 0; i < 3; i++ {
    cert, err = xkms.Certificate(attrs)
