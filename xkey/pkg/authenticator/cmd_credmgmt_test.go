...
// setupPINTokenForCredMgmtV2 sets up a PIN token with credential management permission
// and activeProtocol set to PINProtocol2.
func setupPINTokenForCredMgmtV2(t *testing.T, auth *Authenticator) []byte {
	t.Helper()
	...
	// Initialize PIN protocol state with V2 active protocol.
	auth.pinState.protocol = &pinProtocolState{
		pinUvAuthToken:   make([]byte, 32),
		tokenPermissions: PINPermissionCredentialMgmt,
		activeProtocol:   PINProtocol2,
	}

	return auth.pinState.protocol.pinUvAuthToken
}

// buildCredMgmtRequestV2 builds a credential management request with V2 PIN auth.
// Uses canonical CBOR encoding and full 32-byte HMAC (PINProtocol2).
func buildCredMgmtRequestV2(t *testing.T, pinToken []byte, subCommand uint8, params map[int]interface{}) []byte {
	t.Helper()

	em := credMgmtCanonicalEncoder()

	// Step 1: Build request with placeholder pinAuth (32 bytes for V2).
	placeholderAuth := make([]byte, PINAuthSizeV2)
	req := map[int]interface{}{
		credMgmtKeySubCommand:        subCommand,
		credMgmtKeyPinUvAuthProtocol: PINProtocol2,
		credMgmtKeyPinUvAuthParam:    placeholderAuth,
	}
	if len(params) > 0 {
		req[credMgmtKeySubCommandParams] = params
	...
	mac := hmac.New(sha256.New, pinToken)
	mac.Write(authData)
	correctPinAuth := mac.Sum(nil) // Full 32 bytes for V2
...
// TestCredMgmtEnumerateRPsV2 tests credential management with PINProtocol2 authentication.
func TestCredMgmtEnumerateRPsV2(t *testing.T) {
	auth := createCredMgmtTestAuthenticator(t)
	defer func() { _ = auth.Close() }()
	...
	pinToken := setupPINTokenForCredMgmtV2(t, auth)
	data := buildCredMgmtRequestV2(t, pinToken, CredMgmtEnumerateRPsBegin, nil)
...
// TestCredMgmtV2RejectsTruncatedAuth tests that V2 rejects V1-style truncated 16-byte auth.
func TestCredMgmtV2RejectsTruncatedAuth(t *testing.T) {
	auth := createCredMgmtTestAuthenticator(t)
	defer func() { _ = auth.Close() }()

	pinToken := setupPINTokenForCredMgmtV2(t, auth)

	// Compute correct 32-byte HMAC but truncate to 16 bytes (V1 style).
	authData := []byte{CredMgmtEnumerateRPsBegin}
	mac := hmac.New(sha256.New, pinToken)
	mac.Write(authData)
	truncatedAuth := mac.Sum(nil)[:PINAuthSizeV1] // Wrong: V2 expects full 32 bytes

	req := map[int]interface{}{
		credMgmtKeySubCommand:        CredMgmtEnumerateRPsBegin,
		credMgmtKeyPinUvAuthProtocol: PINProtocol2,
		credMgmtKeyPinUvAuthParam:    truncatedAuth,
	}
	data, err := cbor.Marshal(req)
	require.NoError(t, err)
...
// TestCredMgmtDeleteCredentialV2 tests credential deletion with PINProtocol2 authentication.
func TestCredMgmtDeleteCredentialV2(t *testing.T) {
	auth := createCredMgmtTestAuthenticator(t)
	defer func() { _ = auth.Close() }()
	...
	data := buildCredMgmtRequestV2(t, pinToken, CredMgmtDeleteCredential, params)
