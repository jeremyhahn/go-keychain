...
func (b *AuthDataBuilder) Build() ([]byte, error) {
	...
	// extensions (if ED flag set): CBOR-encoded map using canonical CTAP2 encoder
	if len(b.extensions) > 0 {
		extBytes, err := ctap2EncMode.Marshal(b.extensions)
		if err != nil {
...
func calcCBORLength(data []byte) (int, error) {
	...
	// Re-encode the value using the canonical CTAP2 encoder to get its length.
	// Must use the same encoder as the rest of the authenticator to ensure
	// consistent byte lengths and avoid corrupting authdata boundaries.
