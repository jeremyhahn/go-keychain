...
func parseHMACSecretFromInterfaceMap(m map[interface{}]interface{}) (*HMACSecretInput, error) {
	...
	// Validate saltEnc size: V1 sizes or V2 sizes (V2 has 16-byte IV prefix)
	if len(saltEnc) != HMACSecretEncryptedSingleSaltSize && len(saltEnc) != HMACSecretEncryptedDoubleSaltSize &&
		len(saltEnc) != HMACSecretEncryptedSingleSaltSize+aes.BlockSize && len(saltEnc) != HMACSecretEncryptedDoubleSaltSize+aes.BlockSize {
		return nil, ErrHMACSecretInvalidSaltEnc
	}
	...
	// Accept V1 (16 bytes) or V2 (32 bytes) saltAuth sizes
	if len(saltAuth) != hmacSecretAuthTagSize && len(saltAuth) != PINAuthSizeV2 {
		return nil, ErrHMACSecretInvalidSaltAuth
	}
...
func (a *Authenticator) ProcessHMACSecretExtension(
	...
	// Verify saltAuth: HMAC-SHA-256(sharedSecret, saltEnc) with protocol-aware truncation
	if err := verifySaltAuth(sharedSecret, input.PinUvAuthProtocol, input.SaltEnc, input.SaltAuth); err != nil {
		return nil, err
	}

	// Decrypt saltEnc to get salt(s)
	salts, err := decryptSalts(sharedSecret, input.PinUvAuthProtocol, input.SaltEnc)
	if err != nil {
		return nil, err
	}
	...
	encryptedOutput, err := encryptHMACOutputs(sharedSecret, input.PinUvAuthProtocol, outputs)
	if err != nil {
		return nil, err
	}
...
func deriveHMACSecretSharedSecret(platformCOSEKey []byte, protocol int) ([]byte, error) {
	...
	case PINProtocolVersion2:
		...
		salt := make([]byte, 32)
		hmacKeyReader := hkdf.New(sha256.New, xBytes, salt, []byte("CTAP2 HMAC key"))
		hmacKey := make([]byte, 32)
		if _, err := io.ReadFull(hmacKeyReader, hmacKey); err != nil {
			return nil, ErrHMACSecretKeyDerivationFailed
		}
		return hmacKey, nil
...
// verifySaltAuth verifies the saltAuth HMAC tag with protocol-aware truncation.
// V1: saltAuth = HMAC-SHA-256(sharedSecret, saltEnc)[0:16]
// V2: saltAuth = HMAC-SHA-256(sharedSecret, saltEnc) (full 32 bytes)
func verifySaltAuth(sharedSecret []byte, protocol int, saltEnc, saltAuth []byte) error {
	// Compute expected saltAuth
	mac := hmac.New(sha256.New, sharedSecret)
	mac.Write(saltEnc)
	var expected []byte
	if protocol == PINProtocolVersion2 {
		expected = mac.Sum(nil) // Full 32 bytes for V2
	} else {
		expected = mac.Sum(nil)[:hmacSecretAuthTagSize] // 16 bytes for V1
	}
...
// decryptSalts decrypts the encrypted salt(s) using AES-256-CBC.
// V1: Uses zero IV.
// V2: IV is extracted from the first 16 bytes of saltEnc.
// Returns either 1 salt (32 bytes) or 2 salts (64 bytes).
func decryptSalts(sharedSecret []byte, protocol int, saltEnc []byte) ([]byte, error) {
	...
	// Extract IV and encrypted data based on protocol version
	var iv []byte
	var encrypted []byte
	if protocol == PINProtocolVersion2 {
		if len(saltEnc) < aes.BlockSize {
			return nil, ErrHMACSecretDecryptionFailed
		}
		iv = saltEnc[:aes.BlockSize]
		encrypted = saltEnc[aes.BlockSize:]
	} else {
		iv = make([]byte, aes.BlockSize) // zero IV for V1
		encrypted = saltEnc
	}

	// Validate encrypted data length is a valid CBC ciphertext size
	if len(encrypted) != HMACSecretEncryptedSingleSaltSize && len(encrypted) != HMACSecretEncryptedDoubleSaltSize {
		return nil, ErrHMACSecretInvalidSaltEnc
	}
...
// encryptHMACOutputs encrypts the HMAC output(s) using AES-256-CBC.
// V1: Uses zero IV.
// V2: Generates random IV and prepends it to ciphertext.
func encryptHMACOutputs(sharedSecret []byte, protocol int, outputs []byte) ([]byte, error) {
	// Apply PKCS7 padding
	...
	// Generate IV based on protocol version
	var iv []byte
	if protocol == PINProtocolVersion2 {
