...
func TestBackendStorage_Store(t *testing.T) {
	...
	t.Run("returns ErrStorageError when backend Put fails", func(t *testing.T) {
		...
		s, err := NewBackendStorage(mockBackend, "test/")
...
func TestBackendStorage_Load(t *testing.T) {
	...
	t.Run("returns ErrDeserializationFailed for corrupted data", func(t *testing.T) {
		...
		mockBackend.data["test/credentials/01020304"] = []byte("not valid json{{{")
...
func TestBackendStorage_LoadByRPID(t *testing.T) {
	...
	t.Run("skips indexed credentials when backend Get fails", func(t *testing.T) {
		t.Parallel()
		mockBackend := newMockBackend()

		// Pre-populate backend with a credential before construction
		// so buildIndexAsync populates the RP index
		cred := createTestCredential(t, []byte{0x01}, "example.com", true)
		data, _ := json.Marshal(cred)
		_ = mockBackend.Put(context.Background(), "test/credentials/01", data)
		...
		require.True(t, s.HasCredentialsForRP("example.com"))

		// Now inject a Get error so LoadByRPID can't read the credential
		mockBackend.getErr = errors.New("backend get failure")
	...
	t.Run("skips credentials that cannot be read", func(t *testing.T) {
		t.Parallel()
		mockBackend := newMockBackend()
		// Add a key but with error on get
		mockBackend.keys = []string{"test/credentials/01"}
		mockBackend.getErr = errors.New("cannot read")
	...
	t.Run("skips malformed credentials", func(t *testing.T) {
		t.Parallel()
		mockBackend := newMockBackend()
		// Add a key with invalid JSON
		mockBackend.keys = []string{"test/credentials/01"}
		mockBackend.data["test/credentials/01"] = []byte("invalid json")
...
func TestBackendStorage_Count(t *testing.T) {
	...
	t.Run("returns ErrStorageError when backend List fails", func(t *testing.T) {
		...
		// Set list error after construction so buildIndexAsync has completed
...
func TestBackendStorage_LoadState(t *testing.T) {
	...
	t.Run("returns ErrDeserializationFailed for corrupted state data", func(t *testing.T) {
		t.Parallel()
		mockBackend := newMockBackend()
		mockBackend.data["test/state"] = []byte("not valid json{{{")
...
func TestBackendStorage_Clear(t *testing.T) {
	...
	t.Run("continues deletion even when individual deletes fail", func(t *testing.T) {
		t.Parallel()
		mockBackend := newMockBackend()
		mockBackend.keys = []string{"test/credentials/01", "test/credentials/02"}
		mockBackend.deleteErr = errors.New("delete failed")
		...
		// Clear should not return error even when deletes fail
		err = s.Clear()
		require.NoError(t, err)
	})
}
...
func TestBackendStorage_ListAll(t *testing.T) {
	...
	t.Run("skips keys without expected prefix", func(t *testing.T) {
		t.Parallel()
		mockBackend := newMockBackend()
		// Add keys with and without proper prefix
		mockBackend.keys = []string{
			"test/credentials/01020304",
			"other/credentials/05060708", // different prefix
		}
		mockBackend.data["test/credentials/01020304"] = []byte(`{"credential_id":"AQIDBA=="}`)
		mockBackend.data["other/credentials/05060708"] = []byte(`{"credential_id":"BQYHCg=="}`)
	...
	t.Run("skips keys with invalid hex encoding", func(t *testing.T) {
		t.Parallel()
		mockBackend := newMockBackend()
		// Add keys with valid and invalid hex
		mockBackend.keys = []string{
			"test/credentials/01020304",
			"test/credentials/not-hex-string",
		}
		...
		require.NoError(t, err)
		require.Len(t, credIDs, 1)
		require.Equal(t, []byte{0x01, 0x02, 0x03, 0x04}, credIDs[0])
	})
}
...
// mockBackend is a mock storage backend for testing error paths.
...
func TestBackendStorageRPIndex_BuiltAtInit(t *testing.T) {
	t.Parallel()

	t.Run("index populated from existing credentials", func(t *testing.T) {
		t.Parallel()
		backend := storage.NewMemory()
		defer func() { _ = backend.Close() }()

		// Store credentials directly in the backend before constructing BackendStorage
		s1, err := NewBackendStorage(backend, "test/")
		require.NoError(t, err)
		s1.WaitForIndex()
		require.NoError(t, s1.Store(createTestCredential(t, []byte{1}, "example.com", true)))
		require.NoError(t, s1.Store(createTestCredential(t, []byte{2}, "example.com", false)))
		require.NoError(t, s1.Store(createTestCredential(t, []byte{3}, "other.com", true)))
		require.NoError(t, s1.Close())

		// Create a new BackendStorage on the same backend — index should be rebuilt
		s2, err := NewBackendStorage(backend, "test/")
		require.NoError(t, err)
		s2.WaitForIndex()
		defer func() { _ = s2.Close() }()

		require.True(t, s2.HasCredentialsForRP("example.com"))
		require.True(t, s2.HasCredentialsForRP("other.com"))
		require.False(t, s2.HasCredentialsForRP("unknown.com"))
	})

	t.Run("empty storage produces empty index", func(t *testing.T) {
		t.Parallel()
		s := createBackendStorage(t)
		defer func() { _ = s.Close() }()

		require.False(t, s.HasCredentialsForRP("example.com"))
		require.False(t, s.HasDiscoverableForRP("example.com"))
	})
}

func TestBackendStorageRPIndex_MaintainedOnStore(t *testing.T) {
	t.Parallel()

	t.Run("new credential added to index", func(t *testing.T) {
		t.Parallel()
		s := createBackendStorage(t)
		defer func() { _ = s.Close() }()
		...
		require.NoError(t, s.Store(createTestCredential(t, []byte{1}, "example.com", true)))

		require.True(t, s.HasCredentialsForRP("example.com"))
		require.True(t, s.HasDiscoverableForRP("example.com"))
	})

	t.Run("overwrite updates discoverable flag", func(t *testing.T) {
		t.Parallel()
		s := createBackendStorage(t)
		defer func() { _ = s.Close() }()

		// Store as discoverable
		require.NoError(t, s.Store(createTestCredential(t, []byte{1}, "example.com", true)))
		require.True(t, s.HasDiscoverableForRP("example.com"))

		// Overwrite as non-discoverable
		require.NoError(t, s.Store(createTestCredential(t, []byte{1}, "example.com", false)))
		require.True(t, s.HasCredentialsForRP("example.com"))
		require.False(t, s.HasDiscoverableForRP("example.com"))
	})

	t.Run("multiple RPs indexed independently", func(t *testing.T) {
		t.Parallel()
		s := createBackendStorage(t)
		defer func() { _ = s.Close() }()

		require.NoError(t, s.Store(createTestCredential(t, []byte{1}, "a.com", true)))
		require.NoError(t, s.Store(createTestCredential(t, []byte{2}, "b.com", false)))

		require.True(t, s.HasCredentialsForRP("a.com"))
		require.True(t, s.HasDiscoverableForRP("a.com"))
		require.True(t, s.HasCredentialsForRP("b.com"))
		require.False(t, s.HasDiscoverableForRP("b.com"))
		require.False(t, s.HasCredentialsForRP("c.com"))
	})
}

func TestBackendStorageRPIndex_MaintainedOnDelete(t *testing.T) {
	t.Parallel()

	t.Run("deleted credential removed from index", func(t *testing.T) {
		t.Parallel()
		s := createBackendStorage(t)
		defer func() { _ = s.Close() }()
		...
		require.NoError(t, s.Delete([]byte{1}))
		require.False(t, s.HasCredentialsForRP("example.com"))
	})

	t.Run("deleting one credential preserves others for same RP", func(t *testing.T) {
		t.Parallel()
		s := createBackendStorage(t)
		defer func() { _ = s.Close() }()

		require.NoError(t, s.Store(createTestCredential(t, []byte{1}, "example.com", true)))
		require.NoError(t, s.Store(createTestCredential(t, []byte{2}, "example.com", false)))
	...
	t.Run("deleting last credential removes RP from index", func(t *testing.T) {
		t.Parallel()
		s := createBackendStorage(t)
		defer func() { _ = s.Close() }()
		...
		// RP key should be fully removed, not left as empty slice
		s.mu.RLock()
		_, exists := s.rpIndex["example.com"]
		s.mu.RUnlock()
		require.False(t, exists, "RP key should be removed from index when last credential deleted")
	})
}

func TestBackendStorageLoadByRPID_UsesIndex(t *testing.T) {
	t.Parallel()

	t.Run("only loads matching credentials via index", func(t *testing.T) {
		t.Parallel()
		s := createBackendStorage(t)
		defer func() { _ = s.Close() }()
		...
		require.NoError(t, s.Store(createTestCredential(t, []byte{1}, "example.com", true)))
		require.NoError(t, s.Store(createTestCredential(t, []byte{2}, "example.com", false)))
		require.NoError(t, s.Store(createTestCredential(t, []byte{3}, "other.com", true)))

		// LoadByRPID should use the index to load only the 2 example.com credentials
		creds, err := s.LoadByRPID("example.com")
		require.NoError(t, err)
		require.Len(t, creds, 2)

		for _, cred := range creds {
			require.Equal(t, "example.com", cred.RPID)
		}
	})

	t.Run("returns nil for unknown RPID", func(t *testing.T) {
		t.Parallel()
		s := createBackendStorage(t)
		defer func() { _ = s.Close() }()
...
func TestBackendStorageHasDiscoverableForRP(t *testing.T) {
	t.Parallel()

	t.Run("returns true when discoverable credentials exist", func(t *testing.T) {
		t.Parallel()
		s := createBackendStorage(t)
		defer func() { _ = s.Close() }()
	...
	t.Run("returns false when only non-discoverable credentials exist", func(t *testing.T) {
		t.Parallel()
		s := createBackendStorage(t)
		defer func() { _ = s.Close() }()
	...
	t.Run("returns false for empty storage", func(t *testing.T) {
		t.Parallel()
		s := createBackendStorage(t)
		defer func() { _ = s.Close() }()
	...
	t.Run("returns false for unknown RP", func(t *testing.T) {
		t.Parallel()
		s := createBackendStorage(t)
		defer func() { _ = s.Close() }()

		require.NoError(t, s.Store(createTestCredential(t, []byte{1}, "example.com", true)))
		require.False(t, s.HasDiscoverableForRP("unknown.com"))
	})

	t.Run("mixed discoverable and non-discoverable", func(t *testing.T) {
		t.Parallel()
		s := createBackendStorage(t)
		defer func() { _ = s.Close() }()

		require.NoError(t, s.Store(createTestCredential(t, []byte{1}, "example.com", false)))
		require.NoError(t, s.Store(createTestCredential(t, []byte{2}, "example.com", true)))
		require.NoError(t, s.Store(createTestCredential(t, []byte{3}, "example.com", false)))

		require.True(t, s.HasDiscoverableForRP("example.com"))
	})
}

func TestBackendStorageRPIndex_ClearResetsIndex(t *testing.T) {
	t.Parallel()
	...
	require.NoError(t, s.Store(createTestCredential(t, []byte{1}, "example.com", true)))
	require.NoError(t, s.Store(createTestCredential(t, []byte{2}, "other.com", true)))

	require.True(t, s.HasCredentialsForRP("example.com"))
	require.True(t, s.HasCredentialsForRP("other.com"))

	require.NoError(t, s.Clear())

	require.False(t, s.HasCredentialsForRP("example.com"))
	require.False(t, s.HasCredentialsForRP("other.com"))
}

func TestBackendStorageRPIndex_ConcurrentAccess(t *testing.T) {
	t.Parallel()

	s := createBackendStorage(t)
	defer func() { _ = s.Close() }()

	// Concurrent Store + HasCredentialsForRP to verify thread safety
	var wg sync.WaitGroup
	for i := 0; i < 20; i++ {
		wg.Add(1)
		go func(idx int) {
			defer wg.Done()
			credID := []byte{byte(idx)}
			rpID := "example.com"
			if idx%2 == 0 {
				rpID = "other.com"
			}
			_ = s.Store(createTestCredential(t, credID, rpID, idx%3 == 0))
			_ = s.HasCredentialsForRP(rpID)
			_ = s.HasDiscoverableForRP(rpID)
		}(i)
	}
	wg.Wait()

	// Verify counts are consistent
	exampleCreds, err := s.LoadByRPID("example.com")
	require.NoError(t, err)
	otherCreds, err := s.LoadByRPID("other.com")
	require.NoError(t, err)

	totalCount, err := s.Count()
	require.NoError(t, err)
	require.Equal(t, 20, totalCount)
	require.Equal(t, 20, len(exampleCreds)+len(otherCreds))
}
