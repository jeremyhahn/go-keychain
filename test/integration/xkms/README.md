# XKMS Integration Tests

Comprehensive integration tests for the `pkg/xkms` package - the composite xkms that manages multiple backends.

## Overview

This test suite provides end-to-end integration testing of the xkms package, which provides a unified interface for cryptographic key and certificate management across multiple storage backends.

## Test Coverage

The integration tests achieve **67.3% code coverage** of the `pkg/xkms` package with **126 test cases** covering:

### Core Functionality

#### Key Generation (`xkms_integration_test.go`)
...
```bash
go test -v -tags=integration ./test/integration/xkms/...
```

### Run Specific Test Suite
```bash
go test -v -tags=integration ./test/integration/xkms/... -run TestKeyStore_KeyGeneration
go test -v -tags=integration ./test/integration/xkms/... -run TestKeyID
go test -v -tags=integration ./test/integration/xkms/... -run TestPassword
```

### Run with Coverage
```bash
go test -tags=integration -coverpkg=github.com/jeremyhahn/go-xkms/pkg/xkms \
  -coverprofile=coverage.out ./test/integration/xkms/...
go tool cover -html=coverage.out
```

### Run with Race Detection
```bash
go test -v -tags=integration -race ./test/integration/xkms/...
```
...
#### `createTestKeyStore(t *testing.T) xkms.KeyStore`
...
### `xkms_integration_test.go` (Main Test Suite)
- **15 test functions** covering core xkms workflows
