...
func TestKeyStore_New_ErrorCases(t *testing.T) {
	t.Run("NilBackend", func(t *testing.T) {
		...
		_, err := xkms.New(&xkms.BackendConfig{
			Backend:     nil,
	...
	t.Run("NilCertStorage", func(t *testing.T) {
		_, err := xkms.New(&xkms.BackendConfig{
			Backend:     createTestKeyStore(t).KeyProvider(),
			CertStorage: nil,
		})
		if err == nil {
...
func TestKeyStore_RotateKey_AllAlgorithms(t *testing.T) {
	tests := []struct {
		name      string
		setupFunc func(ks xkms.Backend) (*types.KeyAttributes, error)
	}{
		{
			name: "RSA",
			setupFunc: func(ks xkms.Backend) (*types.KeyAttributes, error) {
...
func TestPassword(t *testing.T) {
	t.Run("NewPassword", func(t *testing.T) {
		...
		password := xkms.NewPassword(passwordBytes)
		...
		if clearPassword, ok := password.(*xkms.ClearPassword); ok {
	...
	t.Run("NewPasswordFromString", func(t *testing.T) {
		passwordStr := "test-password-string"
		password := xkms.NewPasswordFromString(passwordStr)
