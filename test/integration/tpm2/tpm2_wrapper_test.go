...
func TestIntegration_TPM2_ParsePublicKey(t *testing.T) {
	...
	// Get EK public area
	_, ekPub, ekPubErr := tpmInstance.EKPublic()
	if ekPubErr != nil {
		t.Fatalf("EKPublic failed: %v", ekPubErr)
	}
...
func TestIntegration_TPM2_SealUnseal_WithSecret(t *testing.T) {
	...
	// Seal the secret
	_, err = tpmInstance.SealKey(keyAttrs, nil, true)
	if err != nil {
		t.Fatalf("Seal failed: %v", err)
	}

	// Unseal and verify
	unsealed, err := tpmInstance.UnsealKey(keyAttrs, nil)
	if err != nil {
...
func TestIntegration_TPM2_RSAEncryptDecrypt(t *testing.T) {
	...
	ciphertext, err := tpmInstance.RSAEncrypt(handle, name, message)
	if err != nil {
	...
	// Decrypt
	decrypted, err := tpmInstance.RSADecrypt(handle, name, ciphertext)
	if err != nil {
...
func TestIntegration_TPM2_MakeActivateCredential(t *testing.T) {
	...
	credentialBlob, encryptedSecret, secret, err := tpmInstance.MakeCredential(iakAttrs.TPMAttributes.Name, nil)
	if err != nil {
...
func TestIntegration_TPM2_MakeCredential_CustomSecret(t *testing.T) {
	...
	credentialBlob, encryptedSecret, secret, err := tpmInstance.MakeCredential(iakAttrs.TPMAttributes.Name, customSecret)
	if err != nil {
