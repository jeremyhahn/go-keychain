...
# List with details
xkms frost list --verbose

# Output as JSON
xkms frost list --format json
```
...
```bash
xkms frost info <key-id> [options]
```
...
```bash
# Show key info
xkms frost info my-frost-key

# Show with public key
xkms frost info my-frost-key --show-public-key

# Output as JSON
xkms frost info my-frost-key --format json
```
...
```bash
xkms frost delete <key-id> [options]
```
...
xkms frost delete my-frost-key

# Force delete
xkms frost delete my-frost-key --force
```
...
# Alice:
xkms frost round1 --key-id my-key --output round1-alice.json
# Bob:
xkms frost round1 --key-id my-key --output round1-bob.json
# Charlie:
xkms frost round1 --key-id my-key --output round1-charlie.json
...
|----------|-------------|
| `XKMS_CONFIG` | Path to xkms config file |
| `XKMS_FROST_ALGORITHM` | Default algorithm |
| `XKMS_FROST_THRESHOLD` | Default threshold |
| `XKMS_FROST_BACKEND` | Default secret backend |
