# fido2key Architecture

This document describes the architecture of fido2key and its component interfaces.

## Overview

fido2key is a virtual FIDO2/WebAuthn authenticator that exposes itself as a USB HID device via Linux UHID. It consists of several layered components.
...
**Location**: `cmd/fido2key/device.go`
...
A proxy backend (`--backend proxy`) delegates key operations to a remote keychaind service:

```
fido2key → SDK Transport → keychaind → Backend (TPM2, KMS, etc.)
```
