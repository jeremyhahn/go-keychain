...
func main() {
	...
	if *showVersion {
		fmt.Printf("go-keychain MCP server\n")
	...
	// Ensure MCP protocol is enabled
	if !cfg.Protocols.MCP {
		slog.Error("MCP protocol is not enabled in configuration")
		os.Exit(1)
	}
	...
	// Get the MCP server from the unified server
	mcpServer := srv.MCPServer()
	if mcpServer == nil {
		slog.Error("Failed to initialize MCP server")
		os.Exit(1)
	}
	...
	if err := mcpServer.Start(); err != nil {
		slog.Error("Failed to start MCP server", slog.Any("error", err))
		os.Exit(1)
	}

	slog.Info("MCP server started successfully", "port", cfg.Server.MCPPort)
	...
	if err := mcpServer.Stop(); err != nil {
		slog.Error("Error during MCP server shutdown", slog.Any("error", err))
	}
	...
	slog.Info("MCP server stopped successfully")
}
