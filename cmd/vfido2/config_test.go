...
func TestDefaultConstants(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected: "Virtual FIDO2 Key (go-keychain)",
		},
		{
			name:     "default PID file",
			constant: DefaultPIDFile,
			expected: "/var/run/vfido2.pid",
		},
		{
...
func TestDefaultConfig(t *testing.T) {
	...
	if len(cfg.SerialNumber) != 16 {
		// Could also be fallback "VFIDO2-00000000" (15 chars)
		if cfg.SerialNumber != "VFIDO2-00000000" {
	...
	// Verify daemon mode is disabled by default
	if cfg.Daemon {
		t.Error("Daemon should be false by default")
	}

	// Verify default PID file
...
func TestConfigValidate(t *testing.T) {
	tests := []struct {
		...
		{
			name: "interactive mode with daemon mode",
			config: &Config{
				StorageType:       StorageTypeMemory,
				LogLevel:          "info",
				Interactive:       true,
			...
			expectError: ErrInteractiveDaemonConflict,
		},
		{
...
func TestConfigWithAllFields(t *testing.T) {
	cfg := &Config{
		...
		EnablePIN:           true,
		PIN:                 "123456",
		Daemon:              false, // Cannot be true with Interactive
