...
// Exit codes for the fido2key command.
const (
	exitSuccess = 0
	exitError   = 1
)
...
func run() int {
	...
	flag.StringVar(&cfg.StoragePath, "storage-path", "/var/lib/fido2key", "Path for file storage")
	...
	flag.StringVar(&upTimeout, "timeout", "30s", "Timeout for user presence requests")
...
func printVersion() {
	fmt.Printf("fido2key - FIDO2 Key (go-keychain)\n")
