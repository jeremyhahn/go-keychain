...
import (
	...
	tpm2backend "github.com/jeremyhahn/go-keychain/pkg/fido2/authenticator/keybackend/tpm2"
...
var (
	...
	ErrDeviceAlreadyRunning = errors.New("fido2key: device already running")

	// ErrDeviceNotRunning indicates the device event loop is not running.
	ErrDeviceNotRunning = errors.New("fido2key: device not running")

	// ErrStorageCreationFailed indicates the storage backend could not be created.
	ErrStorageCreationFailed = errors.New("fido2key: storage creation failed")

	// ErrAuthenticatorCreationFailed indicates the authenticator could not be created.
	ErrAuthenticatorCreationFailed = errors.New("fido2key: authenticator creation failed")

	// ErrUHIDOpenFailed indicates the UHID device could not be opened.
	ErrUHIDOpenFailed = errors.New("fido2key: UHID open failed")

	// ErrUHIDCreateFailed indicates the virtual HID device could not be created.
	ErrUHIDCreateFailed = errors.New("fido2key: UHID create failed")

	// ErrPINSetFailed indicates the initial PIN could not be set.
	ErrPINSetFailed = errors.New("fido2key: PIN set failed")

	// ErrTPMOpenFailed indicates the TPM device could not be opened.
	ErrTPMOpenFailed = errors.New("fido2key: TPM open failed")

	// ErrTPMBackendCreationFailed indicates the TPM2 key backend could not be created.
	ErrTPMBackendCreationFailed = errors.New("fido2key: TPM2 backend creation failed")
)
...
func NewVirtualFIDO2Device(cfg *Config, logger *slog.Logger) (*VirtualFIDO2Device, error) {
	...
	authConfig := &authenticator.Config{
		...
		RequireUserPresence:        cfg.Interactive, // Interactive mode requires touch even after PIN
...
func (d *VirtualFIDO2Device) Run(ctx context.Context) error {
	...
	// Wire logger into HID handler for operational logging (keepalives, etc.)
...
func createStorage(cfg *Config) (authenticator.StatefulCredentialStorage, error) {
	...
	case StorageTypeFile:
		...
		storage, err := authenticator.NewBackendStorage(backend, "fido2key/")
		if err != nil {
			_ = backend.Close()
			return nil, err
		}
...
func createKeyBackend(cfg *Config, logger *slog.Logger) (keybackend.FIDO2KeyBackend, error) {
	switch cfg.Backend {
	case "tpm2":
		return createTPM2KeyBackend(cfg, logger)
	case "software":
...
// createTPM2KeyBackend opens a TPM connection and creates a TPM2-backed FIDO2 key backend.
func createTPM2KeyBackend(cfg *Config, logger *slog.Logger) (keybackend.FIDO2KeyBackend, error) {
	...
	if err != nil {
		return nil, errors.Join(ErrTPMOpenFailed, err)
	}

	adapter := tpm2backend.NewTPMSignerAdapter(tpm)

	backend, err := tpm2backend.NewTPM2KeyBackend(&tpm2backend.TPM2Config{
		DevicePath: cfg.TPMDevice,
		TPM:        adapter,
	})
	if err != nil {
		_ = adapter.Close()
		return nil, errors.Join(ErrTPMBackendCreationFailed, err)
	}
