...
func main() {
	...
	if *showVersion {
		fmt.Printf("go-keychain gRPC server\n")
	...
	// Ensure gRPC protocol is enabled
	if !cfg.Protocols.GRPC {
		slog.Error("gRPC protocol is not enabled in configuration")
		os.Exit(1)
	}
	...
	// Start the server (which will start gRPC in a goroutine)
	if err := srv.Start(); err != nil {
		slog.Error("Failed to start server", slog.Any("error", err))
		os.Exit(1)
	}

	slog.Info("gRPC server started successfully", "port", cfg.Server.GRPCPort)
