...
// Transport implements the transport.Client interface using gRPC over Unix domain socket.
...
// New creates a new Unix socket gRPC transport with the given options.
...
// NewWithConfig creates a new Unix socket gRPC transport with the given configuration.
...
func (t *Transport) Connect(ctx context.Context) error {
	...
	opts = append(opts, grpc.WithContextDialer(func(ctx context.Context, addr string) (net.Conn, error) {
		return net.Dial("unix", t.config.Address)
	}))
	...
	conn, err := grpc.NewClient("passthrough:///"+t.config.Address, opts...)
	if err != nil {
